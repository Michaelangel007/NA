...
;If all map arrays have an empty record, then get next record)
	...
	LDA MAP_OBJECTS.TRANSPORT+$2,X			;empty record?
	CMP #$00	
...
;INIT VARIABLES (1 OF 2)
 LDA #$00
 STA SPRITE.MANAGE_OBJECT.LOOP.COUNTER	;$00=mob iteration, $01=npc iteration, $02=next map object record
...
SWITCH.MOB_NPC
	LDA SPRITE.MANAGE_OBJECT.LOOP.COUNTER
	CMP #$01							;$00=mob iteration, $01=npc iteration, $02=next map object record
	BEQ .LOAD.NPC.RECORD
	CMP #$02
...
;(test array for an empty record by checking $00 in the 3rd field (tile_type) of the current record.
	...
	LDA MAP_OBJECTS.MOB+$02,X				;empty record?
	CMP #$00	
	BNE .CONTINUE1 					
	JMP INCREMENT.MOB_NPC.COUNTER
...
.NORTH.OR.SOUTH
	...
	CMP SPRITE.RECORD+$09			;X-axis of NPC current position
	BCC .MOVE.NORTH
;.MOVE.SOUTH
...
.MAKE.MOVE
	...
	INC NPC.MOVE.COUNTER
			...
			; ; LDA NPC.PATHFINDER.FINAL.PATH+$1,X
			; ; TAY
			; ; LDA NPC.PATHFINDER.FINAL.PATH,X
			; ; LDX NPC.MOVE.COUNTER
			; LDY #$AA
			; JMP FULL.BRK
			; BRK
...
IDENTIFY.SCREEN_SECTION ;PLAYER IS MAGNET
@START
...
.TILE.NOTVISIBLE
	...
	STA SCREEN.MO_SPRITE.DATA,Y				;SAVE TO SCREEN ARRAY TO MARK MOB'S PRESENCE FOR OTHER ROUTINES LIKE ANIMATION
...
;SAVE CURRENT SPRITE RECORD UPDATES TO THE MAP OBJECTS ARRAY
	...
	LDA SPRITE.MANAGE_OBJECT.LOOP.COUNTER		;$00=mob iteration, $01=npc iteration, $02=next map object record
	CMP #$01 
...
.LOOP									;since SPRITE.RECORD is larger than MAP.OBJECTS.MOB, we need to init SPRITE.RECORD on each iteration or left over data from MAP.OBJECTS.MOB could get leftover. 
	...
	INC SPRITE.MANAGE_OBJECT.LOOP.COUNTER		;$00=mob iteration, $01=npc iteration, $02=next map object record
	JMP SWITCH.MOB_NPC
...
;OR .APPLY.PLAYER_MOVE (TRANSPORT.ENTRANCE) deteced invalid value in PLAYER.MOVE
;OR .APPLY.PLAYER_MOVE (MOB.ENTRANCE) deteced invalid value in PLAYER.MOVE
	LDA TEXT
	LDA $C082
	BRK
