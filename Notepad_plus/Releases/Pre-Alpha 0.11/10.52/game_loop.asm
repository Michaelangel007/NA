...
@START
	...
	JSR KEYIN.ANIMATION
	STA SAVED.ACC.LOCAL	;save direction key for future use
	CMP #$8B			;UP ARROW
	BEQ .CHECK_NORTH
	CMP #$8A			;DOWN ARROW
	BEQ .CHECK_SOUTH
...
.INVALID.COMMAND
;command not valid becuase direction selected is not valid or because a pushable general object does not exist 1 tile from player location in the direction selected
...
.CHECK_NORTH	
	LDX #SCREEN.ARRAY.ADJACENT_NORTH
	LDA #SCREEN.ARRAY.ADJACENT_NORTH2
	JMP .CHECK.OBJECT.TYPE

.CHECK_SOUTH	
	LDX #SCREEN.ARRAY.ADJACENT_SOUTH
	LDA #SCREEN.ARRAY.ADJACENT_SOUTH2
	JMP .CHECK.OBJECT.TYPE

.CHECK_EAST
	LDX #SCREEN.ARRAY.ADJACENT_EAST
	LDA #SCREEN.ARRAY.ADJACENT_EAST2
	JMP .CHECK.OBJECT.TYPE
	
.CHECK_WEST
	LDX #SCREEN.ARRAY.ADJACENT_WEST
...
.CHECK.OBJECT.TYPE	
	...
	LDA SCREEN.MO_GENERAL.DATA,X ;load general map object record index from screen array, location = offset 1 tile from player location in the direction of the push command. 
	TAX			;Transfer map object index from ACC to X-REG
	CMP #$FF					;does adjacent tile contain a general map object?
	BEQ .INVALID.COMMAND		;if not, then push command is not valid
	;IS GENERAL OJECT A TRANSPORT OBJECT?
	CMP #TILE_ID.HORSE_C						;BRANCH BASED ON THE TYPE OF TRASNPORT MO AT THE PLAYER LOCATION
...
;swap object and player location
	LDA SAVED.ACC.LOCAL	;restore ASCII code of direction key pressed
	CMP #$8B			;UP ARROW
	BEQ .SWAP_NORTH
	CMP #$8A			;DOWN ARROW
	BEQ .SWAP_SOUTH
