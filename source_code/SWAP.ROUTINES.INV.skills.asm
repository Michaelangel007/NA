...
;**OPT** Memory. I did a detailed analysis and it would take $B4 bytes to replace .LOAD.SCREEN.VARIABLES thru .LOOP.PRINT_SCREEN
				;with a SCREEN_BUILDER command table. The current code uses $DB bytes so the savings would be
				;$27 byte. And, .SCREEN_TILE could be replaced at a savings of $14 byte.
				;grand total savings = $3B bytes.
...
;PRINT SKILL PROGRESS
	...
	;print variable: skill progress
		...
		; CLC ;(CLC = don't print CR | SEC = print CR to text window)
	JSR PRINT.HEX8.RJ
		;**OPT** Memory. There are a lot of calls to PRINT.HEX8.RJ and PRINT.HEX8.LJ with CLC parm set (don't print CR). 
	...
	;**PLACE HOLDER**
		...
		NOP
		NOP
	JSR INV_3.PLACEHOLDER  ;print "AAA"
