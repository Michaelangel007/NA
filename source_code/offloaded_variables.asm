...
;**Shared Var space**: (TM_1.TREASURE_MGT): SHARED.VARIABLE_SPACE.BLOCK2+$00 - ??
;
...
;
;-----SHAPE.HOPPER0----
;*Used extensively by inventory module. 
;*I think the lower part of the range is used by the character sheet return array
;
;
...
;**above are all verified as fully utilized (I double checked)

SCREEN_HOLE.2578_257F	.EQ	$2578	;$8bytes	
SCREEN_HOLE.25F8_25FF	.EQ	$25F8	;$8bytes	
SCREEN_HOLE.2678_267F	.EQ	$2678	;$8bytes	
...
TILE_ID.MERCHANT.EQ				.EQ $98				;#CONSTANT	
...
TILE_ID.ROAD.LT1				.EQ $5B				;#CONSTANT		TILE TYPE ID
...
PAGE.BACKGROUND				.EQ SCREEN_HOLE.2E78_2E7F+$5	;$1byte. CONTAINS THE OPPOSITE VALUE OF PAGE, SO IT CAN BE USED BY ROUTINES THAT NEED TO DRAW TO THE BACKGROUND PAGE. 

;PAGE.FOREGROUND.OVERRIDE	.BS $1		;1byt 	$01 = override on, $00 = off
PAGE.FOREGROUND.OVERRIDE	.EQ SCREEN_HOLE.2E78_2E7F+$6	;$1byte. 1byt 	$01 = override on, $00 = off

;DRAW.TILE.SHAPE_TABLE.OVERRIDE .BS $1	;($00 = load data into shape table from aux memory | $01 don't load data into shape table)
DRAW.TILE.SHAPE_TABLE.OVERRIDE .EQ SCREEN_HOLE.2E78_2E7F+$7	;$1byte. ($00 = load data into shape table from aux memory | $01 don't load data into shape table)
...
;zpage usage (after REV 201705170-01). arbitrary selection except for the "ProDOS constant" ones  *******$5x variables***
...
; ;zpage usage (before REV 201705170-01)				*******$Fx variables***
...
;BCD.UNPACK & KEYIN.BCD
BCD      		.EQ DIVIDEND32	;$3bytes (I think it might be 4 bytes. See KEYIN.BCD)
;BCD      		.BS $4	;$3bytes
BCD.TOTAL		.EQ OP1+$0	;$1bytes
BCD.MAX			.EQ $04	;#CONSTANT. The max BCD input digits supported by KEYIN.BCD
BCD.SHIFTED		.EQ DIVISOR	;$4bytes

;CONVERT.BCD_TO_HEX & CONVERT.HEX_TO_BCD
BIN								.EQ DIVISOR		;$4byte

BINARY							.EQ BIN			;$4byte
EXP								.EQ OP1+$0
CONVERT.BCD_TO_HEX.TEMP			.EQ OP1+$1
...
;PRINT.HEX16
PRINT.HEX16.PARM.MODE			.EQ OP1 ;($00 = left justify | $01 = right justify)

; ;PRINT.BCD VARIABLES
; BCD.DIGITS 			.BS 1

;SCREEN_BUILDER
SCREEN_BUILDER.COMMAND_TABLE.POINTER			.EQ $BA	;ZPAGE POINTER
SCREEN_BUILDER.LOOKUP_TABLE.POINTER 			.EQ $BC	;ZPAGE POINTER
SCREEN_BUILDER.VARIABLE_POINTER					.EQ SCREEN_BUILDER.LOOKUP_TABLE.POINTER ;ZPAGE POINTER
SCREEN_BUILDER.BOOLEAN_VARIABLE.POINTER			.EQ SCREEN_BUILDER.LOOKUP_TABLE.POINTER ;ZPAGE POINTER

SCREEN_BUILDER.COMMAND_TABLE.SIZE				.EQ SEED0
SCREEN_BUILDER.HTAB.LEFT						.EQ SEED1
SCREEN_BUILDER.COMMAND_CODE						.EQ SEED2
SCREEN_BUILDER.CR.PARM							.EQ SEED3 ;($00 = off | $01 = on)
SCREEN_BUILDER.CUSTOMIZED.LOOKUP_TABLE.INDEX1	.EQ RND.LO
...
SHP.TBL.CNTR					.EQ SCREEN_HOLE.3078_307F+$1	;$1byte.

;TILE.LINE.CNTR					.BS $1
TILE.LINE.CNTR					.EQ SCREEN_HOLE.3078_307F+$2	;$1byte.
...
ANIMATION.CALLED_BY				.EQ SCREEN_HOLE.2078_207F+$0	;$1byte. Used to track the routine which called animation manager. $00 = not tracked (no special handling), $01 = KEYIN.STRING. This variable is reset when ANIMATION.MANAGER exits
...
ANIMATION.FORCED				.EQ SCREEN_HOLE.2078_207F+$1	;$1byte. IF FLAG IS SET ($01), THEN AN ANIMATION ABORT TO DUE PLAYER KEY PRESS IS NOT ALLOWED, FORCING THE FULL DRAW OF THE NEXT ANIMATION FRAME.  
...
ANIMATION.FORCED.OVERRIDE		.EQ SCREEN_HOLE.2078_207F+$2	;$1byte. USED FOR PLAYTESTING. IF FLAG IS SET ($01), THEN AN ANIMATION ABORT TO DUE PLAYER KEY PRESS IS NOT ALLOWED, FORCING THE FULL DRAW OF THE NEXT ANIMATION FRAME.   

;ANIMATION.FRAME_STATE			.BS	$1			;CURRENT ANIMATION FRAME FOR ALL ANIMATION TILES ON THE VIEW SCREEN (0-3)
ANIMATION.FRAME_STATE			.EQ SCREEN_HOLE.2078_207F+$3	;$1byte. CURRENT ANIMATION FRAME FOR ALL ANIMATION TILES ON THE VIEW SCREEN (0-3)
...
ANIMATION.DELAY.OVERRIDE		.EQ SCREEN_HOLE.2078_207F+$4	;$1byte. IF FLAG IS SET ($01) THEN THE ANIMATION DELAY IS SKIPPED. OVERRIDE IS AUTOMATICALLY INIT TO OFF AFTER EACH CALL TO ANIMATION.UPDATE
...
ANIMATION.SCREEN.TALLY			.EQ SCREEN_HOLE.2078_207F+$5	;$1byte. A TALLY OF THE NUMBER OF ANIMATED TILES ON THE VIEW SCREEN. UPDATED AFTER COMPLETING AN ANIMATION FRAME CYCLE. USED TO DETERMINE IF A DELAY SHOULD BE INSERTED TO SMOOTH OUT ANIMATION SPEED
...
ANIMATION.DEEP_WATER.TALLY		.EQ SCREEN_HOLE.2078_207F+$6	;$1byte. A TALLY OF THE NUMEBR OF DEEP WATER TILES ON THE VIEW SCREEN, used to determine whether to force animation to complete when entire screen is deep water tiles
...
ANIMATION.CURRENT_TILE_TYPE		.EQ SCREEN_HOLE.2078_207F+$7	;$1byte. HOLDS THE TILE_TYPE FOR THE CURRENT TILE LOCATION BEING PROCESSED (HOLDS A TILE_TYPE LOADED FROM SCREEN.TILE.DATA OR SCREEN.MO_SPRITE.DATA)
...
ANIMATION.SCROLL.HOPPER 		.EQ SCREEN_HOLE.20F8_20FF+$0	;$2bytes. USED WHEN A BYTES FROM ANOTHER SHAPE ARE COPIED IN TO A SCROLLING TILE (EXAMPLE, PLAYER ICON SINKING IN WATER)
					   	        ;===IN USE==			 +$1
...
ANIMATION.MT.INDEX				.EQ SCREEN_HOLE.20F8_20FF+$2	;$1bytes. HOLDS THE RECORD INDEX TO THE MOB IN THE CURRENT SCREEN LOCATION


;ANIMATION.MT.TILE_NUMBER		.BS	$1			;HOLDS THE CURRENT MT TILE NUMBER OF THE MOB IN THE CURRENT SCREEN LOCATION
ANIMATION.MT.TILE_NUMBER		.EQ SCREEN_HOLE.20F8_20FF+$3	;$1bytes. HOLDS THE CURRENT MT TILE NUMBER OF THE MOB IN THE CURRENT SCREEN LOCATION

;ANIMATION.MT.FINAL_TILE_TYPE	.BS $3		;HOLDS THE FINAL TILE_TYPE FOR MT MOBS, NET OF ANY ADJUSTMENTS FOR THE MT TILE # (0-3)
ANIMATION.MT.FINAL_TILE_TYPE	.EQ SCREEN_HOLE.20F8_20FF+$4	;$3bytes. HOLDS THE FINAL TILE_TYPE FOR MT MOBS, NET OF ANY ADJUSTMENTS FOR THE MT TILE # (0-3)
					   	        ;===IN USE==			 +$5
					   	        ;===IN USE==			 +$6


;ANIMATION.XREG.STORAGE			.BS $1
ANIMATION.XREG.STORAGE			.EQ SCREEN_HOLE.20F8_20FF+$7	;$1bytes.

;ANIMATION.YREG.STORAGE			.BS $1
ANIMATION.YREG.STORAGE			.EQ SCREEN_HOLE.21F8_21FF+$0	;$1byte

;ANIMATION.YREG.STORAGE1		.BS $1
ANIMATION.YREG.STORAGE1			.EQ SCREEN_HOLE.21F8_21FF+$1	;$1byte
...
COMBAT.SPECIAL.TOTAL		.EQ SHARED.VARIABLE_SPACE.BLOCK2+$1A ;$1bytes. The number of specials summoned to the battle (includes specials with all health status codes. i.e. this isn't the number that are alive, it's the number of special records that were created during the battle)
...
COMBAT.DRAW.SINGLE.OFF_GRID.WAIT_VALUE	 .EQ COMBAT.EXPLOSION.START.COLUMN ;$1byte. Parameter value for WAIT.LOOP between drawing the shape and screen animation update.
;COMBAT.DRAW.SINGLE.OFF_GRID.WAIT_VALUE	 .BS $1 ;$1byte. Parameter value for WAIT.LOOP between drawing the shape and screen animation update.
...
TWB.COMBAT.SCROLL_WINDOW.TOP_LINE				.EQ	$6B ;#CONSTANT. 
...
SCREEN.DARK.ALGORITHM.ROW				.EQ SCREEN_HOLE.21F8_21FF+$2	;$1byte

;SCREEN.DARK.ALGORITHM.COLUMN			.BS	$1
SCREEN.DARK.ALGORITHM.COLUMN			.EQ SCREEN_HOLE.21F8_21FF+$3	;$1byte

;SCREEN.DARK.ALGORITHM.LOCATION			.BS $1
SCREEN.DARK.ALGORITHM.LOCATION			.EQ SCREEN_HOLE.21F8_21FF+$4	;$1byte
...
SCREEN.DARK.ALGORITHM.LOCATION_STOP		.EQ SCREEN_HOLE.21F8_21FF+$5	;$1byte
;SCREEN.DARK.ALGORITHM.LOCATION_START	.BS $1
SCREEN.DARK.ALGORITHM.LOCATION_START	.EQ SCREEN_HOLE.21F8_21FF+$6	;$1byte
;SCREEN.DARK.ALGORITHM.LOCATION_CURRENT	.BS $1
SCREEN.DARK.ALGORITHM.LOCATION_CURRENT	.EQ SCREEN_HOLE.21F8_21FF+$7	;$1byte
...
;SHARED.VARIABLE_SPACE.BLOCK2 $C-$1F **available**
								
;new ELS algorithm: offscreen
ELS.OFFSCREEN.SMAP_UL.OFFSET			.EQ	$018B ;#CONSTANT. RMAP value of Upper left tile in SCREEN.TILE.DATA.EXTENDED
ELS.EXTENDED_INDEX.OFFSET.NORTH_SOUTH	.EQ $21 ;#CONSTANT. 


;ELS.OFFSCREEN.ROW_COLUMN.COUNTER	.EQ	SHARED.VARIABLE_SPACE.BLOCK2+$20 ;$1byte
EXTENDED_TILE_INDEX.CURRENT			.EQ	EXTENDED_TILE_INDEX.ELS ;$2byte
							
; EXTENDED_TILE_INDEX.CURRENT			.EQ	SHARED.VARIABLE_SPACE.BLOCK2+$21 ;$2byte
									; ;==IN USE==						+$22
ELS.OFFSCREEN.COLUMN_ROW_SIZE		.EQ	SHARED.VARIABLE_SPACE.BLOCK2+$23 ;$1byte
ELS.OFFSCREEN.SEARCH.TILE_COUNTER	.EQ	SHARED.VARIABLE_SPACE.BLOCK2+$24 ;$1byte
ELS.OFFSCREEN.SEARCH.MODE			.EQ	SHARED.VARIABLE_SPACE.BLOCK2+$25 ;$1byte
ELS.OFFSCREEN.SMAP_UL				.EQ	SHARED.VARIABLE_SPACE.BLOCK2+$26 ;$2byte										
									;==IN USE==						+$27							
...
;DRAWING TILES
TILE.LINE						.EQ SCREEN_HOLE.2178_217F+$0	;$1byte. KEEPS TRACK OF THE CURRENT LINE IN DRAW.TILE
...
IMAGE_DRAW.LINE					.EQ TILE.LINE		;1byte
IMAGE_DRAW.LINE.START			.EQ TILE.LINE.START	;1byte
IMAGE_DRAW.LINE.STOP			.EQ TILE.LINE.STOP	;1byte
...
SCREEN.DRAW.CURRENT_BYTE		.EQ SCREEN_HOLE.2178_217F+$4		;$1byte. KEEPS TRACK OF THE CURRENT SCREEN BYTE IN DRAW.TILE
SCREEN.DRAW.BYTE1				.EQ SCREEN_HOLE.2178_217F+$5		;$1byte. USED IN DRAW.TILE.FOREGROUND TO TRACK SCREEN BYTES INSTEAD OF A REGISTER
SCREEN.DRAW.BYTE2				.EQ SCREEN_HOLE.2178_217F+$6		;$1byte. USED IN DRAW.TILE.FOREGROUND TO TRACK SCREEN BYTES INSTEAD OF A REGISTER

IMAGE_DRAW.STOP_BYTE			.EQ SCREEN.DRAW.BYTE1			;1byte
IMAGE_DRAW.START_BYTE			.EQ SCREEN.DRAW.BYTE2			;1byte
IMAGE_DRAW.CURRENT_BYTE			.EQ SCREEN.DRAW.CURRENT_BYTE	;1byte
...
SCREEN.ARRAY.STOP_VALUE			.EQ SCREEN_HOLE.2178_217F+$7		;$1byte. USED TO DETECTED THE END OF DATA WHEN ITERATING THROUGH A COLUMN IN SCREEN.TILE.DATA
...
SAVED_TILE_TYPE					.EQ SCREEN_HOLE.2278_227F+$0	;$1byte. DRAW.TILE.SINGLE USES THE TILE TYPE STORED IN THIS VARIABLE UNLESS IT IS SET TO $00
...
;DRAW.STOP_BYTE.ALT	.EQ	TILE.LINE.COPY ;DRAW.STOP_BYTE cannot be used if LINE.STOP is used by the same subroutine, so this variable is an alternative. 
DRAW.STOP_BYTE.ALT	.EQ	SCREEN.DRAW.BYTE2 ;DRAW.STOP_BYTE cannot be used if LINE.STOP is used by the same subroutine, so this variable is an alternative. 
...
;=====INVENTORY & CHARACTER SHEETS=====  (we've got a sheet on it)
@START
...
;(I started to use this range below, see INV.SUB_MODULE.LOAD_FLAGS)
...
INV.SHAPE.HOPPER0.OUT_OF_ORDER.E9			.EQ SHAPE.HOPPER0+$E9	;#CONSTANT
INV.SHAPE.HOPPER0.OUT_OF_ORDER.EA			.EQ SHAPE.HOPPER0+$EA		;#CONSTANT
INV.SHAPE.HOPPER0.OUT_OF_ORDER.EB			.EQ SHAPE.HOPPER0+$EB		;#CONSTANT

;GENERAL
INV.SUB_MODULE.LOAD_FLAGS			.EQ	SHAPE.HOPPER0+$16 ;$09 bytes.  ($00 = sub-module not loaded in memory | $01 = sub-module loaded in memory)
;							;SHAPE.HOPPER0+$13-$1F   ====IN USE==== 
...
INV.SUB_MODULE.LOAD_FLAGS.ARRAY_SIZE	.EQ $09
...
INV.MENU_ICON7.SBYTE .EQ	$1D


;ENTRANCE
@START
INV.SUB_MODULE.LAUNCH_CODE			.EQ	SHAPE.HOPPER0+$59 	;$1bytes.   (inventory sub-module # to launch)
INV.ACTIVE_MENU.CODE				.EQ	SHAPE.HOPPER0+$5A 	;$1bytes. ($0-$5 represents the menu icon active, or $0-6 when merchant transaction interface is active)
...
INV.ACTIVE_MENU.CODE.MAX.VARIABLE	.EQ INV.SHAPE.HOPPER0.OUT_OF_ORDER.EB ;$1 bytes. ;tracks the code of the menu on the far right, so it can wrap around to the 1st on the left. A variable is used so it can be different value when INV_8 is active.
;INV.ACTIVE_MENU.CODE.MAX.VARIABLE	.BS $1 ;$1 bytes. ;tracks the code of the menu on the far right, so it can wrap around to the 1st on the left. A variable is used so it can be different value when INV_8 is active.
...
INV.ACTIVE_MENU.MERCHANT_INVENTORY	.EQ $06					;#CONSTANT
...
PLAYER_MERCH.INVENTORY.DATA.POINTER.SAVED	.EQ	SHAPE.HOPPER0+$A2 ;$2bytes.
...
PLAYER_MERCH.INVENTORY.DATA.POINTER 					.EQ $B0 ;2bytes. #POINTER
PLAYER_MERCH.INVENTORY.DATA.SELECTED_RECORD.POINTER		.EQ $B2 ;2bytes. #POINTER
...
;PLAYER_MERCH.INVENTORY.DATA.POINTER			.EQ $B2 ;#ZPAGE POINTER
INV_4.MAX_ONSCREEN.ITEMS 					.EQ $06 ;#CONSTANT. Max number of items that can be displayed in the inventory window
;INV_4.1b.DISPLAY.MODE 							.EQ	SHAPE.HOPPER0+$B7 	;$1bytes ($00 = display weapons | $01 = display armor | $02 = misc items)
INV_4.1b.DISPLAY.MODE 							.BS $1 ;$1bytes ($00 = display weapons | $01 = display armor | $02 = misc items))
INV_4.1b.CALL_FROM 							.BS $1 ;$1bytes ;($00 = subroutine called from INV_4, display player inventory | $01 = subroutine called from INV_8 merchant transactions)
INV_4.SPELL_RANGE.LTH						.EQ $40 ;#CONSTANT
INV_4.1b.SCREEN_ITEM_LIST.ARRAY 				.EQ SHAPE.HOPPER0+$B8	;$18bytes	 
;datagram: (inventory table record # LO address, inventory table record # HO address, QTY_AVAILABLE, readied status ($00 = not readied, $01 = readied, $FF = deleted record))
INV_4.1b.SCREEN_ITEM_LIST.ARRAY.RECORD_SIZE 	.EQ $04 ;#CONSTANT. 
INV_4.1b.SCREEN_ITEM_LIST.ARRAY.ARRAY_SIZE 	= INV_4.1b.SCREEN_ITEM_LIST.ARRAY.RECORD_SIZE*INV_4.MAX_ONSCREEN.ITEMS
INV_4.1b.SCREEN_ITEM_LIST.SELECTED.ITEM		.EQ SHAPE.HOPPER0+$D0	;$1bytes	
INV_4.1b.SCREEN_LIST.TOTAL_RECORDS				.EQ SHAPE.HOPPER0+$D1	;$1bytes. ;(0-5 = item # of last item on screen | $FF = no inventory records are eligible for display). If < #INV_4.MAX_ONSCREEN.ITEMS that means less than #INV_4.MAX_ONSCREEN.ITEMS in the party inventory are relevant for display to the active player 
INV_4.ACTIVE_PLAYER 						.EQ RE.ACTIVE_PLAYER
;INV_4.1a.SCREEN_ITEM_LIST.DATA *****see local variables in INV_4.DISPLAY_PLAYER_INVENTORY
...
INV_4.1b.NEXT_ITEM.QTY_AVAILABLE 				.EQ SHAPE.HOPPER0+$D2	;$1bytes. (quantity of the item in the record found that is available to be readied by the active player)
INV_4.NEXT_ITEM.READIED_STATUS 				.EQ SHAPE.HOPPER0+$D3	;$1bytes. Flag that indicates if the active player has the item (in the record found) readied. ($00 = not readied, $01 = readied).
INV_4.1b.GET.NEXT_ITEM.PARM.MODE 				.EQ SHAPE.HOPPER0+$D4	;$1bytes. ($00 = iterate down | $01 = iterate up)
INV_4.1b.GET_ITEMS.LOOP_COUNTER				.EQ SHAPE.HOPPER0+$D5	;$1bytes.
INV_4.DUAL_WIELDING.FLAG					.EQ SHAPE.HOPPER0+$D6	;$1bytes. ($00 = single wielding | $01 = dual wielding)
; PLAYER_MERCH.INVENTORY.DATA.POINTER.SAVED			.EQ SHAPE.HOPPER0+	;$2bytes.
...
INV_4.GROUP2.PRINT.MISC_TEXT.MODE					.EQ SHAPE.HOPPER0+$DF	;$1bytes.

;screen position constants

;****also see INV_4.1a.SCREEN_LIST_ITEM.LOOKUP_TABLE.VTAB in INV_7 local variables
INV_4.HEADER1.HTAB		.EQ 	$01	;#CONSTANT
INV_4.HEADER1.VTAB		.EQ 	$04	;#CONSTANT

INV_4.1b.SCREEN_LIST.ITEM0.VTAB				.EQ $06		;#CONSTANT.	
...
INV_4.1b.MAX_ONSCREEN.ITEMS					.EQ SHAPE.HOPPER0+$E0	;$1bytes.
INV_4.1b.SCREEN_LIST.ITEM_ALL.HTAB.LEFT		.EQ SHAPE.HOPPER0+$E1	;$1bytes.
;INV_4.1b.HOTKEY_UPPER_PLUS1					.EQ SHAPE.HOPPER0		;$1bytes.
;INV_4.1b.SCREEN_LIST.ITEM_ALL.HTAB.LEFT		.EQ SHAPE.HOPPER0		;$1bytes.
INV_4.1b.SCREEN_LIST.ITEM_ALL.HTAB.RIGHT		.EQ SHAPE.HOPPER0+$E2	;$1bytes.
INV_4.1b.SPELL_READIED.FLAG 					.EQ SHAPE.HOPPER0+$E3	;$1bytes. ;($00 = spell book item IS readied | !=$00 spell book item IS NOT readied)
INV_4.1b.PLAYER_INVENTORY.FILE_DATA.FLAG 		.EQ SHAPE.HOPPER0+$E4	;$1bytes. ;($00 = not in memory | >=$01 in memory)
;INV_4.FIRST.SCREEN_RENDERING					.EQ SHAPE.HOPPER0+ 	;$1bytes. ;($00 = first screen render for the INV_4 session, i.e. when inventory items are first printed onscreen, >=$01 not first screen render for the INV_4 session, i.e. state loop has been reached at least once)

	
;SHAPE.HOPPER0 variable space $E5-$E8 available
...
;INV_8.MERCHANT_TRANSACTIONS (shared variable space with READY/UNREADY EQUIPMENT, $5F-$AF)
@START
;text block screen positions
@START
INV_8.PLAYER_GOLD_LABEL.HTAB		.EQ 	$23	;#CONSTANT
INV_8.PLAYER_GOLD_LABEL.VTAB		.EQ 	$01	;#CONSTANT
INV_8.PLAYER_GOLD_AMOUNT.HTAB		.EQ		$23 ;#CONSTANT
INV_8.PLAYER_GOLD_AMOUNT.VTAB		.EQ 	$02	;#CONSTANT


INV_8.HEADER1.HTAB			.EQ 	$15	;#CONSTANT
INV_8.HEADER1.VTAB			.EQ 	$04	;#CONSTANT

INV_8.TEXT_BLOCK.BUY_SELL.HTAB		.EQ $15 ;#CONSTANT
INV_8.TEXT_BLOCK.BUY_SELL.VTAB		.EQ $F ;#CONSTANT


INV_8.SCREEN_LIST.ITEM_ALL.HTAB.LEFT 		.EQ $15	;#CONSTANT. Should be same value as TWS.MERCHANT_TRANS.SCROLL_WINDOW.LEFT_SBYTE
INV_8.SCREEN_LIST.ITEM_ALL.WT_HTAB			.EQ $24 ;#CONSTANT


INV_8.SCREEN_LIST.ITEM_ALL.HTAB.RIGHT		.EQ $26	;#CONSTANT. Should be same value as TWS.MERCHANT_TRANS.SCROLL_WINDOW.RIGHT_SBYTE

INV_8.SCREEN_LIST.ITEM_ALL.PRICE_HTAB		.EQ $23	;#CONSTANT
...
;screen management
@START
INV_8.ACTIVE_MENU.CODE.MAX					.EQ $06 ;#CONSTANT. Maximum value permitted for this variable
INV_8.MAX_ONSCREEN.ITEMS 					.EQ $03 ;#CONSTANT. Max number of items that can be displayed in the inventory window. This value is a quantity (i.e starts with 1 not 0)

@END

;data structures
@START
;INV_8.MERCHANT.PROFILE.ITEM.DATA					.EQ INV_8.MERCHANT.PROFILE.DATA+INV_8.MERCHANT.PROFILE.ITEM_RECORDS_START.OFFSET
INV_8.BYTE_COUNTER 							.EQ SHAPE.HOPPER0+$5F ;$1bytes.	;counts the byte position of PLAYER_MERCH.INVENTORY.DATA.POINTER in INV_8.MERCHANT.PROFILE.DATA
...
;files
@START 
INV_8.MERCHANT_PROFILE_ID 					.EQ SHAPE.HOPPER0+$60	;$1bytes.
INV_8.MERCHANT_PROFILE_ID.COUNTER 			.EQ SHAPE.HOPPER0+$61	;$1bytes.
INV_8.MERCHANT_PROFILE.SEEK_BYTES			.EQ SHAPE.HOPPER0+$62	;$2bytes.
										;===IN USE====		 +$63
...
;other
@START
INV_8.GET.ITEM_BASE_PRICE.PARM.ITEM_TYPE		.EQ SHAPE.HOPPER0+$64	;$1bytes.
INV__.SEARCH.PLY.INV.ITEM_TYPE					.EQ INV_8.GET.ITEM_BASE_PRICE.PARM.ITEM_TYPE

INV_8.SELECTED.ITEM_TYPE						.EQ INV_8.GET.ITEM_BASE_PRICE.PARM.ITEM_TYPE
INV_8.GET.ITEM_BASE_PRICE.PARM.ITEM_ID			.EQ SHAPE.HOPPER0+$65	;$1bytes.
INV__.SEARCH.PLY.INV.ITEM_ID					.EQ INV_8.GET.ITEM_BASE_PRICE.PARM.ITEM_ID	;$1bytes.

INV_8.SELECTED.ITEM_ID							.EQ INV_8.GET.ITEM_BASE_PRICE.PARM.ITEM_ID

INV_8.GET.ITEM_BASE_PRICE.RETURN.PRICE			.EQ SHAPE.HOPPER0+$66	;$2bytes.
										;===IN USE====			 +$67
INV_8.GET.ITEM_FINAL_PRICE.PARM.PERCENT			.EQ SHAPE.HOPPER0+$68	;$1bytes.	
INV_8.BUY_ITEM.QTY								.EQ SHAPE.HOPPER0+$69	;$1bytes.	
INV_8.SELL_ITEM.QTY								.EQ INV_8.BUY_ITEM.QTY  ;$1bytes

INV_8.APPLY_PERCENT.PARM.PERCENT				.EQ SHAPE.HOPPER0+$6A	;$1bytes.
INV_8.APPLY_PERCENT.PARM_BASE					.EQ SHAPE.HOPPER0+$6B	;$1bytes.

INV_8.PARM.RE.SEARCH.ITEM_TYPE					.EQ SHAPE.HOPPER0+$6C 	;$1bytes.
INV_8.PARM.RE.SEARCH.ITEM_ID					.EQ INV_8.GET.ITEM_BASE_PRICE.PARM.ITEM_ID

;INV_8_4.PLAYER_INV.REFRESH_FLAG				.EQ SHAPE.HOPPER0+ 	;$1bytes. ($00 = refresh mode OFF | >=$01 = refresh mode on). Used when INV_8 wants INV_4 to refresh the player inventory window, without switching menus, after the initial launch. i.e. after an item is purchased. 
INV_8.SELL_ITEM.KNOWN_UNKNOWN.FLAG				.EQ SHAPE.HOPPER0+$6D 	;$1bytes. ($00 = item is unknown | >=$01 = item is known)

;INV_8.1.MASTER_PRICE_TABLE.DATA											
INV_8.PLAYER_GOLD.CAPACITY						.EQ SHAPE.HOPPER0+$6E 	;$2bytes.
										;===IN USE====			 +$6F
INV_8.SELL_ITEM.FINAL_PRICE						.EQ SHAPE.HOPPER0+$70 	;$2bytes.
										;===IN USE====			 +$71

INV_8.SELL_ITEM.PLAYER.KEYPRESS					.EQ SHAPE.HOPPER0+$72 	;$1bytes.	

INV_8.subroutines4.1b_FLAG						.EQ SHAPE.HOPPER0+$73 	;$1bytes. ($00 = not in memory| >=$01 = in memory). 
INV_8.subroutines4.1all_FLAG					.EQ INV_8.subroutines4.1b_FLAG 	;$1bytes. ($00 = not in memory| >=$01 = in memory). This flag shared the same memory as the 4.1b flag so that two STAs aren't needed to init both variables. 
INV_8.NPC_RECORD								.EQ SHAPE.HOPPER0+$74 	;$1bytes. (the map object record number of the NPC the player is talking to, which has been identified as a merchant)
...
;NOTE: INV_4.DISPLAY_PLAYER_INVENTORY copies the master item table records into a buffer for 
;onscreen items. As a result, the offset constants must be are instead the constants below.
;For example, this occurs in .PRINT.LINE1 (INV_4.1a.PRINT.ALL.ITEMS) when printing the item weight.



;common
INV.ITEM_TABLE.ALL.RESIST.MAGIC			.EQ FILE.ITEM_TABLE.RECORD.READ+$0
INV.ITEM_TABLE.ALL.LEVEL				.EQ FILE.ITEM_TABLE.RECORD.READ+$1
INV.ITEM_TABLE.ALL.LEVEL.OFFSET		.EQ $01 ;#CONSTANT
...
INV.ITEM_TABLE.ALL.EQUIPMENT_SLOT.OFFSET	.EQ $C ;#CONSTANT	
...
INV.ITEM_TABLE.AR_MISC.LEVEL					.EQ FILE.ITEM_TABLE.RECORD.READ+$1
INV.ITEM_TABLE.AR_MISC.LEVEL.OFFSET				.EQ $1
...
; INV.ITEM_TABLE.MI.SPELL_DMG_LO.OFFSET			.EQ $09
; INV.ITEM_TABLE.MI.SPELL_DMG_HO.OFFSET			.EQ $0A
INV.ITEM_TABLE.MI.SPELL_DMG.OFFSET				.EQ $0A ;not X2
INV.ITEM_TABLE.MI.OTHER_INFO					.EQ FILE.ITEM_TABLE.RECORD.READ+$B
INV.ITEM_TABLE.MI.OTHER_INFO.OFFSET				.EQ $0B
...
;-mob only
CHR_SHEET.MOB.TREASURE_CODE			.EQ CHR_SHEET.RECORD.READ+$0
CHR_SHEET.MOB.LEVEL					.EQ CHR_SHEET.PC_MOB.LEVEL
...
@START
INV_1.SCREEN1_TITLE.HTAB			.EQ $01 ;#CONSTANT.
INV_1.SCREEN1_TITLE.VTAB			.EQ $04 ;#CONSTANT.
...
;dungeon entrance goes to treasure room (works)
;MAP.WORMHOLES.START.POSITION_DATA	.HS			40.1D.30.32.44.00.32.44.1D.30.40.52.6A.68.4F.00.38.40.52.6A.FF						
...
PARM.GMAP.X				.EQ SCREEN_HOLE.23F8_23FF+$0	;$1byte. stores GMAP.X as a parameter for CONVERT.xxx routines (see map_tools.asm) **SHARED**
...
PARM.GMAP.Y				.EQ SCREEN_HOLE.23F8_23FF+$1	;$1byte. stores GMAP.Y as a parameter for CONVERT.xxx routines (see map_tools.asm)
...
PARM.WZONE				.EQ SCREEN_HOLE.23F8_23FF+$2	;$1byte. stores PLAYER.WMAP.ZONE as a parameter for CONVERT.xxx routines (see map_tools.asm)
...
MAP_OBJECTS.MAP_LOCATION	.EQ SCREEN_HOLE.23F8_23FF+$5	;$2byte. STORES THE GMAP LOCATION A GIVEN MAP OBJECT
							;==IN USE==				 ;$6
...
MOB.MOVE.CURRENT			.EQ SCREEN_HOLE.2578_257F+$5 ;$1byte. IF A MOB DECIDED TO MOVE, THIS VARIABLE HOLDS THE DIRECTION CODE. SAME DIRECTION CODES AS FOR PLAYER MOVEMENT (SEE GAME_LOOP.ASM)
;MOB.MOVE.COUNTER			.BS $1		;COUNTS MOVES FOR DOUBLE-MOVER MOBS
MOB.MOVE.COUNTER			.EQ SCREEN_HOLE.2578_257F+$6 ;$1byte. COUNTS MOVES FOR DOUBLE-MOVER MOBS
...
MOB.MOVE.ERASE_COUNTER		.EQ SCREEN_HOLE.2578_257F+$7 ;$1byte. RECORD WHEN ERASE TILE IS EXECUTED ON A GIVEN TURN FOR A MOB, SO THAT DOUBLE MOVER'S DON'T ERASE TWICE

;MOB.MOVE.OPTIONS_PRIMARY	.BS $5		;SIZED TO $5 SO THE MOVE DECISION ARRAYS CAN BE INIT IN ONE LOOP
MOB.MOVE.OPTIONS_PRIMARY	.EQ SCREEN_HOLE.2578_257F+$0 ;$5byte. SIZED TO $5 SO THE MOVE DECISION ARRAYS CAN BE INIT IN ONE LOOP
							;==IN USE==				+$1-$4
							
;MOB.MOVE.OPTIONS_SECONDARY	.BS $5		;SIZED TO $5 SO THE MOVE DECISION ARRAYS CAN BE INIT IN ONE LOOP
MOB.MOVE.OPTIONS_SECONDARY	.EQ SCREEN_HOLE.25F8_25FF+$0	;$5bytes. SIZED TO $5 SO THE MOVE DECISION ARRAYS CAN BE INIT IN ONE LOOP
							;==IN USE==				+$1-$4

;MOB.MOVE.OPEN_PATHS			.BS $5		;SIZED TO $5 SO THE MOVE DECISION ARRAYS CAN BE INIT IN ONE LOOP
MOB.MOVE.OPEN_PATHS			.EQ SCREEN_HOLE.2678_267F+$0	;$5bytes. SIZED TO $5 SO THE MOVE DECISION ARRAYS CAN BE INIT IN ONE LOOP
							;==IN USE==				+$1-$4

MOB.MOVE.TOTAL_OPEN_PATHS	.EQ SCREEN_HOLE.25F8_25FF+$5	;$1bytes. USED TO GUIDE THE PARSING OF THE RANDOM NUMBER RESULTS USED TO SELECT THE MOVE OPTION
MOB.MOVE.CANDIDATE			.EQ SCREEN_HOLE.25F8_25FF+$6	;$1bytes. USED TO STORE THE MOVE DIRECTION CODE OF THE MOVE THE MOB WANTS TO MAKE, AND WILL MAKE UNLESS BLOCKED.
MOB.MOVE.LAST				.EQ SCREEN_HOLE.25F8_25FF+$7	;$1bytes. RECORDS THE LAST MOVE THE MOB MADE. IN THE INITIAL IMPLEMENTATION, IT WOULD NOT RECORD A PASS AS A MOVE, SO MOBS NEVER PASS TWICE
...
MOB.COLLISION_OVERRIDE		.EQ SCREEN_HOLE.2678_267F+$5	;$1bytes. $00 = OFF, $01 = ON
...
MOB.MOVES.BLOCKED			.EQ SCREEN_HOLE.26F8_26FF+$0	;$5bytes. STORES A VALUE INDICATING WHETHER EACH OF THE TILES ADJACENT TO THE MOB IS BLOCKED ($00 = PERMITTED, $01 BLOCKED)
							;==IN USE==				+$1-$4
...
MOB.ADJACENT_TILES			.EQ SCREEN_HOLE.3078_307F+$4		;4byt. STORES THE TILE #'S OF THE TILES ADJACENT TO THE MOB FOR USE WITH COLLISION CHECKS. BORROWS THE MEMORY USED BY SCREEN.TILE.HOPPER 
							;==IN USE==				 +$5-$7
...
MOB.CANDIDATE.RMAP			.EQ SCREEN_HOLE.2778_277F+$2	;$2byte. STORES THE RANDOM NUMBER GENERATED FOR A NEW MOB'S LOCATION, BEFORE THE LOCATION HAS BEEN CHECKED FOR COLLISION
							;==IN USE==				;+$3
;MOB.CANDIDATE.TYPE			.BS $1		;STORES THE TILE_TYPE VALUE (BYTE2 OF THE MOB RECORD) FOR A NEW MOB BEING CREATED
MOB.CANDIDATE.TYPE			.EQ SCREEN_HOLE.2778_277F+$4	;$1byte. STORES THE TILE_TYPE VALUE (BYTE2 OF THE MOB RECORD) FOR A NEW MOB BEING CREATED
;MOB.CANDIDATE.FLAGS		.BS $1		;STORES THE FLAG VALUE (BYTE3 OF THE MOB RECORD) FOR A NEW MOB BEING CREATED
MOB.CANDIDATE.FLAGS			.EQ SCREEN_HOLE.2778_277F+$5	;$1byte. STORES THE FLAG VALUE (BYTE3 OF THE MOB RECORD) FOR A NEW MOB BEING CREATED
...
MOB.CANDIDATE.MAP.X			.EQ SCREEN_HOLE.2778_277F+$6	;$1byte. STORES CANDIDATE MOB'S MAP X-AXIS LOCATION. DIFFERENT THAN THE X,Y USED TO STORE MOB'S RELATIVE POSITION TO PLAYER
...
MOB.CANDIDATE.MAP.Y			.EQ SCREEN_HOLE.2778_277F+$7	;$1byte. STORES CANDIDATE MOB'S MAP X-AXIS LOCATION. DIFFERENT THAN THE X,Y USED TO STORE MOB'S RELATIVE POSITION TO PLAYER

	

;MOB.CANDIDATE.RELATIVE.X	.BS $1		;STORES CANDIDATE MOB'S RELATIVE (TO PLAYER) X-AXIS LOCATION. 
MOB.CANDIDATE.RELATIVE.X	.EQ SCREEN_HOLE.27F8_27FF+$0	;$1byte. STORES CANDIDATE MOB'S RELATIVE (TO PLAYER) X-AXIS LOCATION. 
;MOB.CANDIDATE.RELATIVE.Y	.BS $1		;STORES CANDIDATE MOB'S RELATIVE (TO PLAYER) Y-AXIS LOCATION.
MOB.CANDIDATE.RELATIVE.Y	.EQ SCREEN_HOLE.27F8_27FF+$1	;$1byte. STORES CANDIDATE MOB'S RELATIVE (TO PLAYER) Y-AXIS LOCATION.
...
PLAYER.MOVE.CURRENT 			.EQ SCREEN_HOLE.29F8_29FF+$0 ;$1byte. CONTAINS THE DIRECTION OF PLAYERS REQUESTED MOVE, AFTER #COLLISION CHECKS. 
...
PLAYER.MOVE.JUMP				.EQ SCREEN_HOLE.29F8_29FF+$1 ;$1byte. CONTAINS $01 IF THE PLAYER EXECUTES A JUMP COMMAND WHILE ON A HORSE
...
PLAYER.MOVE.COUNTER				.EQ SCREEN_HOLE.29F8_29FF+$2 ;$1byte. TRACKS WHETHER PLAYER MOVE 1 OR 2 IS BEING PROCESSED WHEN FAST HORSE IS ACTIVE. NEEDED WHEN (J)UMP COMMAND IS USED, WHICH TEMPORARILY ENABLES FAST HORSE. 
...
PLAYER.MOVE.FORCED				.EQ SCREEN_HOLE.29F8_29FF+$3 ;$1byte. $01 == FORCED. USED WHEN MOVE.N/S/E/W ARE CALLED IN SITUATIONS OTHER THNA A PLAYER KEYPRESS TRIGGER (I.E. TO USE THEM TO MOVE PLAYERS LOCATION AUTOMATICALLY FOR SOME REASON, SUCH AS WHEN AN FRIGATE (MT OBJECT) IS BOARDED AND THE SHIP ISN'T CENTERED ON THE SCREEN. 
;PLAYER.TILE.LAST				.BS $1					;THE TILE TYPE THE PLAYER WAS STANDING ON AT THE END OF THE LAST MOVE
PLAYER.TILE.LAST				.EQ SCREEN_HOLE.29F8_29FF+$4 ;$1byte. THE TILE TYPE THE PLAYER WAS STANDING ON AT THE END OF THE LAST MOVE
...
;GAME.SCROLL_SPEED					.EQ SCREEN_HOLE.29F8_29FF ;$1byte. the length of the delay inserted between HRCG text characters printed in the combat scroll window. 		
...
PLAYER.TRANSPORT.STATUS				.EQ SCREEN_HOLE.2A78_2A7F+$6	;$1byte. $00 = NOT SET, $02 = FAST HORSE, $04 = WYVERN  (THIS VARIABLE EXISTS SO THAT PLAYER.TRANSPORT.SPEED CAN BE USED BY OTHER COMMANDS (LIKE JUMP) WITHOUT LOOSING KNOWLEDGE OF TRANSPORT STATUS (I.E. FAST HORSE ENABLED) WHEN THE OTHER COMMAND WAS ISSUED)
...
PLAYER.TRANSPORT.MT.TILE_LOCATIONS	.EQ SCREEN_HOLE.2B78_2B7F+$0	;$4byte. STORES THE SCREEN LOCATIONS OF ANY ACTIVE MULTI TILE TRANSPORT. THE LOCATIONS VERY BASED ON WHICH TILE OF THE MO THE PLAYER BOARDS. 
									;==IN USE==			   $1-$3
...
PLAYER.MAP.X				.EQ SCREEN_HOLE.2AF8_2AFF+$2	;$1byte. STORES PLAYER'S MAP X-AXIS (DIFFERENT THAN THE X,Y USED FOR TRACKING MOB LOCATIONS)
;PLAYER.MAP.Y				.BS $1		;STORES PLAYER'S MAP X-AXIS (DIFFERENT THAN THE X,Y USED FOR TRACKING MOB LOCATIONS)
PLAYER.MAP.Y				.EQ SCREEN_HOLE.2AF8_2AFF+$3	;$1byte. STORES PLAYER'S MAP X-AXIS (DIFFERENT THAN THE X,Y USED FOR TRACKING MOB LOCATIONS)
...
PLAYER.COMMAND.LAST			.EQ SCREEN_HOLE.2AF8_2AFF+$4	;$1byte. STORES THE UPPER CASE ASCII VALUE OF THE LAST VALID COMMAND ISSUED BY THE PLAYER. THIS VALUE IS SET AT THE END OF THE SUBROUTINE FOR THE ASSOCIATED COMMAND. 
;PLAYER.COMMAND.CURRENT		.BS $1
PLAYER.COMMAND.CURRENT		.EQ SCREEN_HOLE.2AF8_2AFF+$5	;$1byte. 
...
PLAYER.GOLD.HTAB		.EQ $24		;#CONSTANT
PLAYER.GOLD.VTAB		.EQ $01		;#CONSTANT
...
SCREEN.COLUMN.STOP			.EQ SCREEN_HOLE.29F8_29FF+$7	;$1byte. STORES THE STOP VALUE WHEN LOADING A COLUMN OF TILES (TILE.LOOKUP.COLUMN)
...
RMAP.LOOKUP					.EQ SCREEN_HOLE.2B78_2B7F+$6	;$2byte. ;USED TO STORE AN RMAP VALUE FOR PURPOSES OF LOOKING UP A TILE TYPE USING TILE.LOOKUP.SINGLE OR THE IN-LINE CODE EQUIVILENT
							;==IN USE==				 +$7
...
;TEXT WINDOW: COMBAT SCROLL WINDOW
;(see combat section. Seach for ";COMBAT SCROLL WINDOW"
...
;TEXT WINDOW: LEFT (PLAYER INVENTORY)
...
;TEXT WINDOW: RIGHT (SCROLL) - MAIN INVENTORY MODULE
...
;TEXT WINDOW: RIGHT (MERCHANT INVENTORY)

;border dimensions
TWB.LW.MERCH_INV.TOP_LINE			.EQ $04	;#CONSTANT.
TWB.LW.MERCH_INV.BOTTOM_LINE		.EQ $BD	;#CONSTANT.
TWB.LW.MERCH_INV.LEFT_SBYTE			.EQ $14	;#CONSTANT. left edge screen byte
TWB.LW.MERCH_INV.RIGHT_SBYTE		.EQ $28 ;#CONSTANT. right edge screen byte
TWB.LW.MERCH_INV.SEPERATOR_LINE		.EQ $1C	;#CONSTANT.

;text space dimensions
TWS.LW.MERCH_INV.TOP_LINE						.EQ $28	;#CONSTANT. The top line below the column headings
TWS.LW.MERCH_INV.BOTTOM_LINE					.EQ $67	;#CONSTANT.
TWS.LW.MERCH_INV.LEFT_SBYTE						.EQ $15	;#CONSTANT. left edge screen byte
TWS.LW.MERCH_INV.RIGHT_SBYTE					.EQ $27 ;#CONSTANT. right edge screen byte


;TEXT WINDOW: RIGHT (SCROLL) - MERCHANT TRANSACTIONS SUB_MODULE

;border dimensions
TWB.MERCHANT_TRANS.SCROLL_WINDOW.TOP_LINE			.EQ	$84 ;#CONSTANT. This is the top line if there entire space were used as a scroll window (i.e. it's the top line of the combat window scroll window). See TWS.INVENTORY.SCROLL_WINDOW.SEPERATOR.LINE
TWB.MERCHANT_TRANS.SCROLL_WINDOW.LEFT_SBYTE			.EQ $14	;#CONSTANT. left edge of top line
TWB.MERCHANT_TRANS.SCROLL_WINDOW.RIGHT_SBYTE		.EQ $27 ;#CONSTANT. right edge screen byte
TWB.MERCHANT_TRANS.SCROLL_WINDOW.BOTTOM_LINE		.EQ $BC	;#CONSTANT.


;text space dimensions
TWS.MERCHANT_TRANS.SCROLL_WINDOW.TOP_LINE				.EQ $88	;#CONSTANT.	Top hi-res line
TWS.MERCHANT_TRANS.SCROLL_WINDOW.BOTTOM_LINE			.EQ $B8	;#CONSTANT.	Top hi-res line
TWS.MERCHANT_TRANS.SCROLL_WINDOW.LEFT_SBYTE				.EQ $15	;#CONSTANT.
TWS.MERCHANT_TRANS.SCROLL_WINDOW.RIGHT_SBYTE			.EQ $26 ;#CONSTANT. right edge screen byte
TWS.MERCHANT_TRANS.SCROLL_WINDOW.TOP_ROW				.EQ $11 ;#CONSTANT		
TWS.MERCHANT_TRANS.SCROLL_WINDOW.BOTTOM_ROW				.EQ $16 ;#CONSTANT		
TWS.MERCHANT_TRANS.SCROLL_WINDOW.WIDTH					.EQ TWS.MERCHANT_TRANS.SCROLL_WINDOW.RIGHT_SBYTE-TWS.MERCHANT_TRANS.SCROLL_WINDOW.LEFT_SBYTE+$1	;#CONSTANT
TWS.MERCHANT_TRANS.SCROLL_WINDOW.CURSOR_START_SBYTE		.EQ TWS.MERCHANT_TRANS.SCROLL_WINDOW.LEFT_SBYTE ;#CONSTANT
TWS.MERCHANT_TRANS.SCROLL_WINDOW.CURSOR_START_ROW		.EQ TWS.MERCHANT_TRANS.SCROLL_WINDOW.TOP_ROW ;#CONSTANT



;TEXT WINDOW: RIGHT (TREASURE DROP)

;border dimensions
TWB.TREASURE_DROP.TOP_LINE			.EQ $04	;#CONSTANT.
TWB.TREASURE_DROP.BOTTOM_LINE		.EQ $BD	;#CONSTANT.
TWB.TREASURE_DROP.LEFT_SBYTE		.EQ $14	;#CONSTANT. left edge screen byte
TWB.TREASURE_DROP.RIGHT_SBYTE		.EQ $27 ;#CONSTANT. right edge screen byte


;text space dimensions
;TWS.TREASURE_DROP.SCROLL_WINDOW.TOP_LINE				.EQ $68	;#CONSTANT. The top line below the column headings
TWS.TREASURE_DROP.SCROLL_WINDOW.TOP_LINE				.EQ $6B	;#CONSTANT. The top line below the column headings
;TWS.TREASURE_DROP.SCROLL_WINDOW.TOP_LINE.TEXT_SPACE	.EQ $20	;#CONSTANT. The top hi-res line of the text row just below the text row that the line is in that seperates the menu icons from the text space. 
TWS.TREASURE_DROP.SCROLL_WINDOW.BOTTOM_LINE				.EQ $BC	;#CONSTANT.
TWS.TREASURE_DROP.SCROLL_WINDOW.LEFT_SBYTE				.EQ $15	;#CONSTANT. left edge screen byte
TWS.TREASURE_DROP.SCROLL_WINDOW.RIGHT_SBYTE				.EQ $26 ;#CONSTANT. right edge screen byte

TWS.TREASURE_DROP.SCROLL_WINDOW.TOP_ROW					.EQ $0E ;#CONSTANT		
TWS.TREASURE_DROP.SCROLL_WINDOW.BOTTOM_ROW				.EQ $16 ;#CONSTANT	
TWS.TREASURE_DROP.SCROLL_WINDOW.WIDTH					.EQ TWS.TREASURE_DROP.SCROLL_WINDOW.RIGHT_SBYTE-TWS.TREASURE_DROP.SCROLL_WINDOW.LEFT_SBYTE+$1	;#CONSTANT
TWS.TREASURE_DROP.SCROLL_WINDOW.CURSOR_START_SBYTE		.EQ TWS.TREASURE_DROP.SCROLL_WINDOW.LEFT_SBYTE ;#CONSTANT
TWS.TREASURE_DROP.SCROLL_WINDOW.CURSOR_START_ROW		.EQ TWS.TREASURE_DROP.SCROLL_WINDOW.TOP_ROW ;#CONSTANT



@END

;========TEXT WINDOW WRAPPER ROUTINES======
@START
;INIT.TEXT_WINDOW.RIGHT
INIT.TEXT_WINDOW.RIGHT.MODE 	.EQ SHAPE.HOPPER0+$00	;($00 = draw input window line | >=$01 = don't draw input window line)
...
;========TREASURE MGT================== (sub_module of SRTN.INVENTORY)
@START
TM_1.LEVEL_GROUP.POINTER					.EQ		$B0		;$2bytes. POINTER

TM_1.DROPPED_ITEM.ITEM_TYPE 				.EQ	SHARED.VARIABLE_SPACE.BLOCK2+$00 ;$1 byte
;TM_1.DROPPED_ITEM.ITEM_CODE 				.EQ SHARED.VARIABLE_SPACE.BLOCK2+$ ;$1 byte
TM_1.DROPPED_ITEM.ITEM_CODE					.EQ FILE.ITEM_TABLE.ID	;$1 byte (uses shape hopper)
TM_1.PLAYER_INVENTORY.FILE_DATA.FLAG 		.EQ SHARED.VARIABLE_SPACE.BLOCK2+$01 ;$1 byte ($00 = not in memory | >=$01 in memory)
TM_1.PICKUP_ALL.FLAG						.EQ SHARED.VARIABLE_SPACE.BLOCK2+$02 ;$1 byte ($00 = not set, >=$01 = set)

TM_1.ITEM_DROP.PROBABILITY.ADJ				.EQ $02		;#CONSTANT. Amount by which the treasure code field on the mob character sheet is divided by to determine the probability of the mob dropping an item				

TM_1.APPLY_PERCENT.PARM.PERCENT				.EQ INV_8.APPLY_PERCENT.PARM.PERCENT
TM_1.APPLY_PERCENT.PARM_BASE				.EQ INV_8.APPLY_PERCENT.PARM_BASE
TM_1.LEVEL_GROUP							.EQ SHARED.VARIABLE_SPACE.BLOCK2+$03
TM_1.GOLD_DROP.TALLY						.EQ SHARED.VARIABLE_SPACE.BLOCK2+$04 ;2 bytes
							 ;====IN USE======  SHARED.VARIABLE_SPACE.BLOCK2+$05
...
;============TIME & EVENTS=============
@START
TIME.SUN.STATUS				.BS $1		;$00 = SUN RISING, $01 = DAY, $02 = SUN SETTING, $03 = NIGHT. **OPT** Memory. Screenhole. Can be converted to screenhole once screen clear happens before GAME.SETUP.DRIVER & once LOADER.P no longer runs at $2000 (I'm planning on moving it to $9600)
...
AUX.READ_ADDRESS.START 			.EQ	SCREEN_HOLE.2BF8_2BFF+$1	;$2byte. 
								;==IN USE==				 +$2

DEBUG.LOG						.EQ $3000 ;***HARD CODED REFERENCE**

;STACK_POINTER.SAVED 			.BS $1
STACK_POINTER.SAVED 			.EQ	SCREEN_HOLE.2BF8_2BFF+$3	;$1byte. 
STACK_POINTER.SAVED_AUX		.BS $1 ;**OPT** Memory. convert to screen hole
...
TROUBLESHOOTING.HOOK			.EQ	SCREEN_HOLE.2BF8_2BFF+$4	;$1byte. $01 = HOOK SET. CODE CAN BE INSERTED IN A ROUTINE TO BREAK IF THE HOOK IS SET.

;TROUBLESHOOTING.HOOK2			.BS $1				;$01 = HOOK SET. CODE CAN BE INSERTED IN A ROUTINE TO BREAK IF THE HOOK IS SET.
TROUBLESHOOTING.HOOK2			.EQ	SCREEN_HOLE.2BF8_2BFF+$5	;$1byte. $01 = HOOK SET. CODE CAN BE INSERTED IN A ROUTINE TO BREAK IF THE HOOK IS SET.
...
CALLED_BY.DRAW.SCREEN			.EQ	SCREEN_HOLE.2BF8_2BFF+$6	;$1byte. 			IF SET TO $01, INDICATES THAT DRAW.SCREEN IS THE CALLING ROUTINE. THIS VARAIBLES IS SEPERATE BECAUSE FLOW CONTROL DECISIONS ARE MADE BASED ON IT. 
...
;***keep*** (the LDA/STAs to set this parm are commented out but could be used again if needed)
...
;COW  .BS $1				;**OPT** Memory. 
COW  	.EQ	SCREEN_HOLE.2BF8_2BFF+$7	;$1byte. 			;**OPT** Memory. 
;COW2 .BS $6				;**OPT** Memory. 
COW2	.EQ SCREEN_HOLE.2C78_2C7F+$0 	;$6byte
		;==IN USE==				 +$1-5


;FILE.OPEN.INVENTORY.DEBUG_COUNTER .BS $1
FILE.OPEN.INVENTORY.DEBUG_COUNTER	.EQ SCREEN_HOLE.2C78_2C7F+$6 	;$1byte
