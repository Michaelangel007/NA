...
;=================================================================================
	...
	BNE .NOT_BLOCKED				;IS MOVE FORCED FLAG SET?. IF YES THEN SKIP COLLISION CHECKS.
...
;COLLISSION CHECK
	...
	JSR PLAYER.COLLISSION.CHECK
		;RETURN VALUE OF $00 = MOVE PERMITTED, $01 = BLOCKED
	CMP #$00							
	BEQ .NOT_BLOCKED
	JMP BLOCKED			
...
;also see .COMMON.UPDATE.PLAYER.MAP_POSITION
	
	
; ;UPDATE SMAP/SMAP.CURRENT	;**OPT** See if this code block (appears in all four routines N/S/E/W) can be moved to the common code section for all movement directions. It looks like this code is identical in each section
	; ;SMAP/SMAP.CURRENT = RMAP(2) - SCREEN.OFFSET.LO/HO (2)
	; LDA RMAP+$0
	; SEC
	; SBC #OFFSET.SCREEN	
	; STA SMAP+$0
...
;(this is so the tranport MO moves with the player)
	...
	CPX #$FF									;IS PLAYER WALKING?  (this check can't be done BMI because this variable also can contain an index to the transport object boarded, which can exceed $7F)
...
.COMMON.UPDATE.PLAYER.MAP_POSITION
;UPDATE SMAP/SMAP.CURRENT	
	;SMAP/SMAP.CURRENT = RMAP(2) - SCREEN.OFFSET.LO/HO (2)
	LDA RMAP+$0
	SEC
...
.EXIT
	LDA ANIMATION.FORCED.OVERRIDE
	BNE .KEYCHECK					;IS ANIMATION FORCED OVERRIDE FLAG SET? IF YES, FORCE THE ABORT KEY PRESS CHECK	
	LDA ANIMATION.FORCED
	BNE .EXIT.SKIP.KEYCHECK			;IS ANIMATION FORCED FLAG SET?. IF YES, DON'T ALLOW AN ABORT DUE TO KEY PRESS		
	LDA PLAYER.MOVE.FORCED			;SET TO $01 BY NON-KEYPRESS ROUTINES TO MOVE PLAYER.
	BNE .EXIT.SKIP.KEYCHECK			;IS MOVE FORCED FLAG SET? IF YES, DON'T ALLOW AN ABORT DUE TO KEY PRESS
.KEYCHECK
	LDA KB_BUFFER					;LOAD NEXT KEY PRESS FROM BUFFER. WAS KEY PRESSED?
    BMI .EXIT.ALTERNATE				;IF YES, SKIP COPY.SCREEN WHICH IS PREP FOR ANIMATION. 
.EXIT.SKIP.KEYCHECK
