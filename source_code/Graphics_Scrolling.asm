...
;*See INV_4.1a.SYNC_PAGES for version of COPY.SCREEN that can be used for just a part of the screen. 
...
;IS LINE COMPLETE?
	...
	BCS .LINE.COMPLETE				;IF YES, SWITCH OVER TO NEXT ROW (BCS: is Y-REG >= CPY value)
		;**OPT** Speed. Memory. The CPY/BEQ can probably be removed if the constant is set to +1 from it's current value. Don't change
						;the constant as something else may be using it. Either add +1 to the expression or setup a new constant. 
.COPYLOOP_STEP	
	INY		
	JMP .COPYLOOP					
