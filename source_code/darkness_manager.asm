...
;Definitions 
;EXTENDED TILE INDEX = index to SCREEN.TILE.DATA.EXTENDED
;TILE INDEX = index to SCREEN.TILE.DATA
;REAL ROW = row # in SCREEN.TILE.DATA
;EXTENDED ROW = row # in SCREEN.TILE.DATA.EXTENDED

-Init
	*set SCREEN.TILE.DATA.EXTENDED INDEX = upper left onscreen section	
-Iterate SCREEN.TILE.DATA(TILE INDEX)
	*If tile not ELS then Next_TILE_INDEX
		*Next_TILE_INDEX
			*TILE INDEX++ (SCREEN.TILE.DATA)
			(skip) *EXTENDED INDEX++ 
			
	-Tile is ELS
		*Calculate EXTENDED TILE INDEX of the ELS
			*Get REAL_ROW.CURRENT_POSITION
			*ELS.EXTENDED_TILE_INDEX = ARRAY CONVERSION OFFSET + REAL ROW * 16 
		*Calculate REAL ROW of light pattern start (upper left tile of light pattern)
			*REAL_ROW.CURRENT_POSITION = REAL_ROW.CURRENT_POSITION - 6

		-Set light pattern
			PARM: REAL_ROW.CURRENT_POSITION
			-Init
				*EXTENDED_ROW.CURRENT_POSITION = REAL_ROW.CURRENT_POSITION + 6
				*LIGHT_PATTER.LOOP_COUNTER = $00
			-Iterate light pattern (start in upper left)
								
				-Calculated LIGHT_POSITION.EXTENDED_TILE_INDEX
					*If LIGHT_PATTERN.LOOP_COUNTER < $13
								THEN: LIGHT_POSITION.EXTENDED_TILE_INDEX = ELS.EXTENDED_TILE_INDEX - LIGHT_PATTERN.OFFSET_TABLE(Y)
								ELSE: LIGHT_POSITION.EXTENDED_TILE_INDEX = ELS.EXTENDED_TILE_INDEX + LIGHT_PATTERN.OFFSET_TABLE(Y)
				...
				-Is light position onscreen?
					...
					*If EXTENDED_ROW.CURRENT_POSITION >= 11 then offscreen
					...
					-is LIGHT.EXTENDED_TILE_INDEX within onscreen range for the real row the light position is in?
						*Use REAL_ROW.CURRENT_POSITION as index to lookup table with LO/HI EXTENDED INDEX of the onscreen range for each row
