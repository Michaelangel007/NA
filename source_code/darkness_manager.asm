...
DARKNESS.REVIEW.AUX_BSR.ENTRANCE ;=============REWORK DARNKESS FOR ENTIRE SCREEN (ENTRANCE)=======
@START
;PARAMETERS: NONE
;RETURN VALUE: NONE
;ENTRANCE: DIRECT
...
; .TEMP
	...
	JSR DARKNESS.PLS
	JSR DARKNESS.ELS
	JSR DARKNESS.SCREEN.UPDATE	;ERASE ANY TILES, NET OF SUN AND LIGHT SOURCES, THAT ARE DARK NOW BUT WEREN'T DARK BEFORE				
	JSR DARKNESS.OBS
...
;SAVE REGISTERS (ENTRY VALUES)
	...
	BEQ .SUN_RISE
.ERROR
;UNEXPECTED VALUE IN TIME.SUN.STATUS	 (DARKNESS.TOD)
	;LDA #$A8
	JSR FULL.BRK
...
;SAVE REGISTERS (ENTRY VALUES)
	...
	PHA
			...
			;SHOE HORN IN FIRE AS ELS ON SURFACE MAP
			; LDA PLAYER.MAP.LOCATION_TYPE
			; STA TEMPORARY.VARIABLE
...
ELS.ONSCREEN.SEARCH.LOOP
	...
	JMP .LOCATION_TYPE.BUILDING.TILES		;if yes	
...
;
;Definitions 
;EXTENDED_TILE_INDEX = index to SCREEN.TILE.DATA.EXTENDED
...
; -Iterate SCREEN.TILE.DATA(TILE INDEX)
	; *If tile not ELS then ELS.NEXT_TILE_INDEX
		; -ELS.NEXT_TILE_INDEX
			; *TILE_INDEX++ 
;			
	; -Tile is ELS
		; *Calculate EXTENDED_TILE_INDEX of the ELS
			; *Get REAL_ROW.CURRENT_POSITION (existing lookup table)
			; *EXTENDED_TILE_INDEX.ELS = TILE_INDEX+EXTENDED.CONVERSION.OFFSET + (REAL ROW * !16)
		; *Calculate EXTENDED ROW of light pattern start (upper left tile of light pattern)
			; *EXTENDED_ROW.CURRENT_POSITION = REAL_ROW.CURRENT_POSITION +6
		; *Call ELS.SET_LIGHT_PATTERN
		; *Loop Until End-Of-Array (SCREEN.TILE.DATA)
;
; -ELS.SET_LIGHT_PATTERN (subroutine)
	; PARMAMETERS: EXTENDED_TILE_INDEX.ELS, EXTENDED_ROW.CURRENT_POSITION
	; -Init Loop
		; *LIGHT_PATTERN.LOOP_COUNTER = $00
	; -Iterate light pattern (start in upper left
		; -Init Iteration
;					
		; -Calculate LIGHT_POSITION.EXTENDED_TILE_INDEX
			; *If LIGHT_PATTERN.LOOP_COUNTER < $13
						; THEN: LIGHT_POSITION.EXTENDED_TILE_INDEX = ELS.EXTENDED_TILE_INDEX - LIGHT_PATTERN.OFFSET_TABLE(LIGHT_PATTERN.LOOP_COUNTER)
						; ELSE: LIGHT_POSITION.EXTENDED_TILE_INDEX = ELS.EXTENDED_TILE_INDEX + LIGHT_PATTERN.OFFSET_TABLE(LIGHT_PATTERN.LOOP_COUNTER)
;
		; -Is light position onscreen?
			; *If EXTENDED_ROW.CURRENT_POSITION < 6 then offscreen
			; *If EXTENDED_ROW.CURRENT_POSITION >= 11 then offscreen
;			
			; -is LIGHT.EXTENDED_TILE_INDEX within onscreen range for the real row the light position is in?
				; *Use EXTENDED_ROW.CURRENT_POSITION-$6 (to convert it to real row) as index to lookup table with EXTENDED INDEX of the left edge of the onscreen range for each row
...
;					
		...
		; *SET SCREEN.DARK.DATA(LIGHT_POSITION.TILE_INDEX) = $01
		; *LIGHT_PATTERN.LOOP_COUNTER++
		; -Increment EXTENDED_ROW.CURRENT_POSITION
			; *LDA EXTENDED_ROW.CURRENT_POSITION
				; CLC
				; ADC EXTENDED_ROW.INCREMENT.LOOKUP_TABLE(LIGHT_PATTERN.LOOP_COUNTER)	;this lookup table will contain the row increment for each position in the light pattern. So 0,0,0,1,0 etc.
				; STA EXTENDED_ROW.CURRENT_POSITION
		; *Loop Until End-Of-Array (LIGHT_PATTERN.OFFSET_TABLE)
;
;=========================================================================(END PSEUDO CODE)		
		
		
.INIT
	;YREG = index to SCREEN.TILE.DATA

;Calculate EXTENDED_TILE_INDEX of the ELS
;(*EXTENDED_TILE_INDEX.ELS = TILE_INDEX+EXTENDED.CONVERSION.OFFSET + (REAL ROW * !16)   )
	
	;calculate (REAL ROW * !16) 
	STY TILE_INDEX.ELS
	LDA SCREEN.INDEX.ROW,Y ;load REAL ROW of ELS
	STA REAL_ROW.CURRENT_POSITION
	ASL ;X2
	ASL ;X4
	ASL ;X8
	ASL ;X16
	CLC
	ADC #EXTENDED.CONVERSION.OFFSET
	STA EXTENDED_TILE_INDEX.ELS+$0
	LDA /EXTENDED.CONVERSION.OFFSET
	ADC #$00 ;16-bit add
	STA EXTENDED_TILE_INDEX.ELS+$1
...
;Calculate EXTENDED ROW of light pattern start (upper left tile of light pattern)	
	...
	STA EXTENDED_ROW.CURRENT_POSITION
			...
			; LDA REAL_ROW.CURRENT_POSITION
			; STA $BF06
			; LDA #EXTENDED_ROW.CONVERSION.OFFSET-3
			; STA $BF07
			; LDA EXTENDED_ROW.CURRENT_POSITION
			; STA $BF08
			; LDA #$AA
...
; -Iterate Offscreen rows
	; *Set SMAP.CURRENT = ELS.OFFSCREEN.SMAP_UL.OFFSET(2) + ELS.RMAP.LOOKUP_TABLE to ELS.RMAP.LOOKUP_TABLE(ELS.OFFSCREEN.ROW_COLUMN.COUNTER)
	; *Get offscreen row of tiles
	; -Get starting EXTENDED_TILE_INDEX.CURRENT for row search
...
;
	...
	; *set ELS.OFFSCREEN.COLUMN_ROW_SIZE = row size constant
	; *call ELS.OFFSCREEN.SEARCH (set parm ACC (row mode))
...
.GET.OFFSCREEN.TILES
	...
	BNE .COLUMN_MODE
.ROWS_MODE
	;NOT parms of TILE.LOOKUP.OFFSCREEN.ROWS. This is a parm of ELS.OFFSCREEN.SEARCH.ROW_COLUMN and is set here to take advantage of the row/column mode branch	
	LDA #SCREEN.ROW.SIZE	
...
.COLUMN_MODE	
	...
	;NOT parms of TILE.LOOKUP.OFFSCREEN.COLUMNS. This is a parm of ELS.OFFSCREEN.SEARCH.ROW_COLUMN and is set here to take advantage of the row/column mode branch
	LDA #$06	;when searching columns, the extended row # is always the same. 
	;**FALLS THROUGH**
.GET.OFFSCREEN.TILES.DONE
	STA EXTENDED_ROW.CURRENT_POSITION
...
; .LOOP.SEARCH.COLUMN
	; LDY SAVED.YREG.LOCAL1			;restore row counter, y-reg is used in the calculation part of the loop, but .els.found	needs y-reg to contains the row counter value. after the first iteration this line is needed to refresh the y-reg value as the row counter. 
...
; .ERROR
; ;UNEXPECTED VALUE IN Y-REG (OFFSCREEN COLUMN COUNTER), LOOP.SEARCH.OFFSCREEN.LEFT
	; ;LDA #$A9
	; JSR FULL.BRK
...
; .ERROR
; ;UNEXPECTED VALUE IN Y-REG (OFFSCREEN COLUMN COUNTER), LOOP.SEARCH.OFFSCREEN.RIGHT
	; ;LDA #$AA
	; JSR FULL.BRK
...
; .ERROR
; ;UNEXPECTED VALUE IN Y-REG (OFFSCREEN COLUMN COUNTER), LOOP.SEARCH.OFFSCREEN.TOP
	; ;LDA #$AB
	; JSR FULL.BRK
...
; .ERROR
; ;UNEXPECTED VALUE IN Y-REG (OFFSCREEN COLUMN COUNTER), LOOP.SEARCH.OFFSCREEN.BOTTOM
	; ;LDA #$AC
	; JSR FULL.BRK
...
.INIT.EXTENDED_INDEX
;Calculated LIGHT_POSITION.EXTENDED_TILE_INDEX
	LDA EXTENDED_TILE_INDEX.ELS+$0
	;XREG: LIGHT_PATTERN.LOOP_COUNTER
	CPX #LIGHT_PATTERN.HALF_SIZE
	BCC .IN.LOWER_HALF
;IN.UPPER_HALF ;current light position is in the upper half of the light pattern
;(LIGHT_POSITION.EXTENDED_TILE_INDEX = ELS.EXTENDED_TILE_INDEX + LIGHT_PATTERN.OFFSET_TABLE(LIGHT_PATTERN.LOOP_COUNTER))		
	CLC
	ADC LIGHT_PATTERN.OFFSET_TABLE,X
	STA LIGHT_POSITION.EXTENDED_TILE_INDEX+$0
	LDA EXTENDED_TILE_INDEX.ELS+$1
	ADC #$00 ;16-bit add
	
	JMP .INIT.EXTENDED_INDEX.DONE
	
; .IS.OFFSCREEN_STEP
	; JMP .IS.OFFSCREEN
	
.IN.LOWER_HALF ;current light position is in the lower half of the light pattern
;(LIGHT_POSITION.EXTENDED_TILE_INDEX = ELS.EXTENDED_TILE_INDEX - LIGHT_PATTERN.OFFSET_TABLE(LIGHT_PATTERN.LOOP_COUNTER))
	SEC
	SBC LIGHT_PATTERN.OFFSET_TABLE,X
	STA LIGHT_POSITION.EXTENDED_TILE_INDEX+$0
	LDA EXTENDED_TILE_INDEX.ELS+$1
...
.INIT.EXTENDED_INDEX.DONE
	STA LIGHT_POSITION.EXTENDED_TILE_INDEX+$1
				...
				; ; CMP #$04
				; ; BNE .TEMP
				; CPX #$24  ;LIGHT_PATTERN.LOOP_COUNTER
				; BNE .TEMP
				; LDA EXTENDED_TILE_INDEX.ELS+$0
				; STA $BF00
				; LDA EXTENDED_TILE_INDEX.ELS+$1
				; STA $BF01
				; LDA LIGHT_PATTERN.OFFSET_TABLE,X
				; STA $BF02
				; LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$0
				; STA $BF03
				; LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$1				
				; STA $BF04
				; LDA #$AA
...
.ONSCREEN_CHECK		;Is light position onscreen?

		; LDA #$80
		; STA COW+0
;check for offscreen top/bottomus
	LDA EXTENDED_ROW.CURRENT_POSITION 
	CMP #EXTENDED_ROW.ONSCREEN_BOUNDARY.TOP
	BCC .IS.OFFSCREEN
	;BCC .IS.OFFSCREEN_STEP	
;.COW ;STEP above only needed to fit debugging code
	CMP #EXTENDED_ROW.ONSCREEN_BOUNDARY.BOTTOM
	BCS .IS.OFFSCREEN
	;BCS .IS.OFFSCREEN_STEP
;.COW ;STEP above only needed to fit debugging code
...
.CORNER_CASE.DONE
	...
	SEC
	SBC LIGHT_POSITION.ONSCREEN.LOOKUP_TABLE,Y	;contains the EXTENDED INDEX (LO byte only) of the left edge of the onscreen range for the row the current light position is in
	BCC .IS.OFFSCREEN	;if an underflow occurs (clear carry), then the current light position is offscreen to the left
	CMP	#ELS.ONSCREEN.ROW_WIDTH
	BCS .IS.OFFSCREEN	;if the current light position - the left edge of the onscreen area is >= the width then it's offscreen to the right
...
;Convert LIGHT_POSITION.EXTENDED_TILE_INDEX to a SCREEN.TILE.DATA index, which is compatile with SCREEN.TILE.DARK
; *LIGHT_POSITION.TILE_INDEX =  LIGHT_POSITION.EXTENDED_TILE_INDEX - ARRAY CONVERSION OFFSET - (REAL_ROW.CURRENT_POSITION * !16)
	
	;(REAL ROW * !16)
	;LDA REAL_ROW.CURRENT_POSITION
	TYA ;Y-REG was converted to REAL ROW above in "check for offscreen left/right"
	ASL ;X2
	ASL ;X4
	ASL ;X8
	ASL ;X16
	STA EXTENDED.CONVERSION.ROW_OFFSET
	
	;subtract EXTENDED.CONVERSION.OFFSET
	;(only LO byte needed because the the HO byte will always equal $00 after both subtractions. We can be sure of this because
	;SCREEN.DARK.DATA only has $BA elements, an 8-bit value)
	LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$0
	SEC
	SBC #EXTENDED.CONVERSION.OFFSET
	SEC ;reset carry flag so we can do another subtraction on the LO byte without an underflow from the prior subtraction affecting the result
	SBC EXTENDED.CONVERSION.ROW_OFFSET
	;STA LIGHT_POSITION.TILE_INDEX 
	

				; ;**TROUBLESHOOTING HOOK**
				; STA TEMP
				; STA LIGHT_POSITION.EXTENDED_TILE_INDEX+$0
				; LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$1	;HO byte not needed because SCREEN.TILE.DATA is 8-bit
				; SBC #$00 ;16-bit subtract
				; STA LIGHT_POSITION.EXTENDED_TILE_INDEX+$1 
				; LDA TROUBLESHOOTING.HOOK
				; CMP #$01
				...
				; STA $BF00
				; lda cow+$1 ;start  LIGHT_POSITION.EXTENDED_TILE_INDEX+$1
				; STA $BF01
				; STY $BF02 ;REAL_ROW.CURRENT_POSITION
				; LDA EXTENDED.CONVERSION.ROW_OFFSET
				; STA $BF03
				; LDA #EXTENDED.CONVERSION.OFFSET
				; STA $BF04
				; LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$0		;FINAL conversion: screen.dark.data index
				; STA $BF05
				; LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$1
				; STA $BF06
				; LDA #$AA
...
;Set Light Position = ON

	;YREG: SCREEN.TILE.DATA index
	TAY 
	LDA #$00
	STA SCREEN.DARK.DATA,Y 	;($00 = visible, $01 = hidden (dark))
...
.INCREMENT.COUNTERS_INDEXES
	...
	LDA EXTENDED_ROW.CURRENT_POSITION
	CLC
	ADC EXTENDED_ROW.INCREMENT.LOOKUP_TABLE,X	;this lookup table will contain the row increment for each position in the light pattern. So 0,0,0,1,0 etc.
	STA EXTENDED_ROW.CURRENT_POSITION
		
	INX ;increment LIGHT_PATTERN.LOOP_COUNTER
...
LIGHT_POSITION.ONSCREEN.LOOKUP_TABLE	.HS CC.ED.0E.2F.50.71.92.B3.D4.F5.16
...
ELS.OFFSCREEN.EXTENDED_TILE_INDEX.LOOKUP_TABLE	
									.HS	69.00.8A.00.AB.00.37.02.58.02.79.02		;searched rows
									.HS	C9.00.CA.00.CB.00.DD.00.DE.00.DF.00		;searched columns	

									
;table holds the extended row of the starting position of each searched row in the order which they will be searched 
ELS.OFFSCREEN.EXTENDED_ROW.LOOKUP_TABLE
									.HS	03.04.05.11.12.13		;searched rows. Columns aren't included in this table because 
																;when searching columns, the extended row # is always the same. 
...
;FUNCTION: ERASE ANY TILES, NET OF SUN AND LIGHT SOURCES, THAT ARE DARK NOW BUT WEREN'T DARK BEFORE
...
;DRAW ANY TILES (NOT DARK NOW, BUT WAS DARK BEFORE) NOT ALREADY DRAWN
;(THE OUTER/INNER LOOPS BELOW DON'T EXAMINE THE TILES ON THE EDGES OF THE SCREEN FOR PURPOSES OF DRAWING.
; THE DRAW.ROW/COLUMN.SINGLE ROUTINES TAKE CARE OF THAT BUT ONLY ONE IS CALLED FOR A GIVEN MOVE.
...
; BUT THE TOP AND BOTTOM ROW NEED TO BE REVIWED FOR TILES WHICH ARE NOT DARK NOW BUT WERE DARK BEFORE
	...
	BEQ .MOVE.EAST_WEST_STEP
	CMP #$04		;PASS
;	BEQ .EXIT_STEP	;if pass, then exit. ideally the screen edges would get reviewed for darkness too, but for now I'm just having it exit. 
	BEQ .MOVE.NORTH_SOUTH
	JMP .ERROR
...
.SPECIFIC.TILE_SET_TYPES
	;CHECK BUILDING TILE SET
	LDA PLAYER.MAP.LOCATION_TYPE
	CMP #MAP.TYPE.SURFACE			;IS LOCATION TYPE = SURFACE?
	BEQ .IN.SURFACE
	; CMP #MAP.TYPE.TOWN_VILLAGE			;IS LOCATION TYPE = BUILDING?
	; BEQ .IN.BUILDING
	CMP #MAP.TYPE.UNDERMAP			;IS LOCATION TYPE = UNDERMAP?
	BEQ .IN.UNDERMAP
	;JMP NEXTTILE
...
.IN.BUILDING	
	LDA TEMP							;RESTORE TILE TYPE
	CMP #DARK_FLAGS.GRE100
	BCC	.IN.BUILDING.OR.UNDERMAP ;branch if tile is not in obscruing range
	CMP #DARK_FLAGS.LT101
	BCS	.IN.BUILDING.OR.UNDERMAP ;branch if tile is not in obscruing range
	JMP DARKNESS.CALCULATE

.IN.UNDERMAP
	LDA TEMP							;RESTORE TILE TYPE
	CMP #DARK_FLAGS.GRE200
	BCC .IN.UNDERMAP.TEST1.COMPLETE
	CMP #DARK_FLAGS.LT200
	BCS	.IN.UNDERMAP.TEST1.COMPLETE
	JMP DARKNESS.CALCULATE

.IN.UNDERMAP.TEST1.COMPLETE
	LDA TEMP							;RESTORE TILE TYPE
	CMP #DARK_FLAGS.GRE201
	BCC .IN.UNDERMAP.COMPLETE
	CMP #DARK_FLAGS.LT201
	BCS	.IN.UNDERMAP.COMPLETE
	JMP DARKNESS.CALCULATE	
.IN.UNDERMAP.COMPLETE
...
.IN.BUILDING.OR.UNDERMAP
	...
	BEQ .CHECK.WINDOW_CELL.DOOR		;if yes, then check to see if it is a window door, but if not it is an obscuring tile
	CMP #MO.DOOR.CLOSED.GRE			;is there a closed door on current tile?
	BCC .NEXT.CHECK					;if no, then continue with other checks
	CMP #MO.DOOR.CLOSED.LT			;is there a closed door on current tile?	
...
.CHECK.WINDOW_CELL.DOOR
	...
	;IS IT A CELL DOOR?
	CMP #TILE_ID.CELL_DOOR.GRE1		;is door a cell door?
	BCC .CHECK.WINDOW.DOOR			;if no, then continue with door analysis. 
	CMP #TILE_ID.CELL_DOOR.LT1		;is door a cell door?
	BCC .NEXT.CHECK					;if yes, then continue with other checks. cell doors are see-through
	;**FALLS THROUGH				;if no, then continue with door analysis
.CHECK.WINDOW.DOOR	
	;IS IT A WINDOW DOOR?
	CMP #TILE_ID.WINDOW_DOOR.GRE1	;is door a window door?
	BCC .DOOR.IS.OBSCURING			;if no, then door is an obscuring tile
	CMP #TILE_ID.WINDOW_DOOR.LT1	;is door a window door?
	BCS .DOOR.IS.OBSCURING			;if no, then door is an obscuring tile

	;IS PLAYER ADJACENT TO WINDOW DOOR?
	CPY #SCREEN.ARRAY.ADJACENT_NORTH	;is door adjacent to player?
	BEQ	.NEXT.CHECK					;if yes, then door is not obscuring, continue with other checks
	CPY #SCREEN.ARRAY.ADJACENT_SOUTH	;is door adjacent to player?
	BEQ	.NEXT.CHECK					;if yes, then door is not obscuring, continue with other checks
	CPY #SCREEN.ARRAY.ADJACENT_EAST	;is door adjacent to player?
	BEQ	.NEXT.CHECK					;if yes, then door is not obscuring, continue with other checks
	CPY #SCREEN.ARRAY.ADJACENT_WEST	;is door adjacent to player?
	BEQ	.NEXT.CHECK					;if yes, then door is not obscuring, continue with other checks
	;**FALLS THROUGH				;if no (door is not adjacent to player), then door is an obscuring tile. 
