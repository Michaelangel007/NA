...
.INIT
	; LDA #$00
	; STA EXTENDED_TILE_INDEX.ELS+$1 ;(init of +$0 isn't needed as it's first value is set by STA)
...
;(*EXTENDED_TILE_INDEX.ELS = TILE_INDEX+EXTENDED.CONVERSION.OFFSET + (REAL ROW * !16)   )
	...
	;ROL EXTENDED_TILE_INDEX.ELS+$1	;the "ASL X16" can cause an overflow, transfered a 1 into the carry flag. The ROL shifts the carry flag into the right side of the HO byte, performing cheap 16-bit multiplication
	
	;add #EXTENDED.CONVERSION.OFFSET
...
;
; -Init
	; *set ELS.OFFSCREEN.SMAP_UL(2) = upper left RMAP value of SCREEN.TILE.DATA.EXTENDED 
				...
				; This is done so that the RMAP start position of all ROW/COLUMN lookups is an offset that can be added to ELS.OFFSCREEN.SMAP_UL.
...
; *LIGHT_POSITION.TILE_INDEX =  LIGHT_POSITION.EXTENDED_TILE_INDEX - ARRAY CONVERSION OFFSET - (REAL_ROW.CURRENT_POSITION * !16)
	...
	;ROL EXTENDED.CONVERSION.ROW_OFFSET+$1	;the "ASL X16" can cause an overflow, transfered a 1 into the carry flag. The ROL shifts the carry flag into the right side of the HO byte, performing cheap 16-bit multiplication
	...
	LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$1 ;**OPT** Memory. This variable isn't being used....do we need this calculation? It seems like we can just ignore the HO Byte
				;sta cow+$1
	SBC #$00 ;16-bit subtract
	STA LIGHT_POSITION.EXTENDED_TILE_INDEX+$1

	;subtract EXTENDED.CONVERSION.ROW_OFFSET
	LDA LIGHT_POSITION.EXTENDED_TILE_INDEX+$0	
	SEC
