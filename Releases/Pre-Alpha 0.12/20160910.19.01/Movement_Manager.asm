...
; .TEMP
	...
	BEQ .WALKING.TILE.TESTS			;if no, then continue with other checks
...
.LOOP	
	...
	LDX SCREEN.MO_GENERAL.DATA,Y	;load general map object data for current tile location
	CPX #$FF						;is a general map object present?
	BEQ .HORSE.WALKING.TILE.TESTS	;if no, then continue with other checks
...
;	
;	Except for buildings (which use a default start position), add the starting postion of the new map to MAP.LOCATIONS.START.POSITION_DATA array
;
;
...
;IS PLAYER ENTERING THE UNDERMAP?
	LDA MAP.LOCATIONS.RECORD+$3		;load map type of the map player is trying to enter
	CMP #LOCATION.TYPE.UNDERMAP		;is it the map type "undermap"?
...
;IS THIS A BUILDING-BUILDING ENTER/EXIT?  (i.e. an upper/lower floor of the same location?)
	LDA MAP.LOCATIONS.RECORD+$3 ;load destination map type
	CMP #LOCATION.TYPE.BUILDING	;is player entering a building type map?
	BNE .UPDATE.LAST.LOCATION.DATA
	LDA PLAYER.MAP.LOCATION_TYPE	 ;load the map type of the destination map (which player is exiting)
	CMP #LOCATION.TYPE.BUILDING	;is player entering a building type map from a building type map (i.e. an upper/lower floor of the same location?)
...
;net time the player exists a building type map via zone transition (LOCATION.EXIT.CHECK)	
...
;the map code of the entrance/exit trigger activated (MAP.LOCATIONS.RECORD)
...
;CALCULATE RMAP.X/Y OF STARTING POSITION
		LDA MAP.LOCATIONS.SP.RECORD+$3	;contains GMAP.X on destination map
		STA PARM.GMAP.X
		STA GMAP.X
		LDA MAP.LOCATIONS.SP.RECORD+$4	;contains GMAP.Y on destination map
		STA PARM.GMAP.Y
		STA GMAP.Y
