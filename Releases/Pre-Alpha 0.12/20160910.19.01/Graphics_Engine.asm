...
;**OPT** Memory. This section may be able to be greatly reduced if it used the DRAW.LINE subroutine in my graphics libary instead of all of the inline code. But consider that setting the parameters for DRAW.LINE will take memory too. Consider how much savings there will really be. 
	LDA TW.RIGHT_WINDOW.CLEAN_UP.FLAG  	;is flag set? This cleanup is only needed the first time DRAW.SCREEN is run after the routine using the text window exists
	CMP #$01
	BNE .DRAW.GENERAL_WINDOW.TEXT
	;**FALLS THROUGH**

	
;DRAW TEXT WINDOW BORDER (RIGHT SIDE): TOP LINE										
.DRAW.TW.BORDER.TOP
		LDA PAGE.FOREGROUND
		LDX #TWB.RIGHT_WINDOW.TOP_LINE
	...
	; ;ACC = PAGE.FOREGROUND
	; CMP #$02	;is page 2 in the foreground?
	; BEQ .SET.PAGE1
; .SET.PAGE2
	; LDA	LINE.BASE.ADDR1
	; STA LINE.BASE.ADDR2
...
.DRAW.GENERAL_WINDOW.TEXT ;/aka the small right side window (4 chars wide)
@START
	;JMP .SKIP
...
.TEXT3 .AZ -/ <Text Window> /			;ASCII text string
.TEXT3.PRINT
		LDA #.TEXT3					
		STA STRING
...
.TEXT4 .AZ -/ <Text Window> /			;ASCII text string
.TEXT4.PRINT
		LDA #.TEXT4					
		STA STRING
