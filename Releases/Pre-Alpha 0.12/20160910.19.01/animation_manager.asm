...
;Note2: using x-reg to preserve acc (current tile type)
	...
	CPX #$01						;is it active?
	BNE .CHECK.FOR.MOB				;if no, then next check
	
		
	LDX SCREEN.INDEX.COLUMN,Y		;load the column number of the current tile
	CPX #TWB.RIGHT_WINDOW.LEFT_TILE_COLUMN	;is the current tile column number >= the left edge of the text window?
...
;	STX SAVED.XREG.LOCAL		;save tile_type
...
.CHECK.FOR.MOB
	LDX SCREEN.MO_SPRITE.DATA,Y			;load the sprite record # for the current tile from the sprite screen array		
	CPX #$FF							;does current tile have a mob mo?
	...
	;alter frame order?
	LDX MOB.FLAG6		;using X-REG to keep ACC unmodified
	BEQ .CALCULATE.SHAPE.TABLE.ADDRESS	;is flag set? If no, then use use the value in ACC for current animation frame when calculating shape table address
		;if flag set, alter frame order to (2,3,0,1.....so real frame 0 = modified frame 2, real frame 1 = modified frame 3, real frame 2 = modified frame 0, real frame 3 = modified frame 1)
	...
	BEQ .FRAME03
.FRAME00
	LDA #$2	;modify current frame to frame 2
	JMP .CALCULATE.SHAPE.TABLE.ADDRESS	;now that the current frame is determine, proceed to calculate the shape table address
	
.FRAME01
	LDA #$3	;modify current frame to frame 3
	JMP .CALCULATE.SHAPE.TABLE.ADDRESS	;now that the current frame is determine, proceed to calculate the shape table address

.FRAME02
	LDA #$0	;modify current frame to frame 0
	JMP .CALCULATE.SHAPE.TABLE.ADDRESS	;now that the current frame is determine, proceed to calculate the shape table address

.FRAME03
	LDA #$1	;modify current frame to frame 1
	;**FALLS THROUGH**
...
.LOAD.SPRITE.LOCATION
	LDA MAP_OBJECTS.MOB+$0,X	;load sprite GMAP X-axis
	STA SPRITE.RECORD+$8		;save

	LDA MAP_OBJECTS.MOB+$1,X	;load sprite GMAP Y-axis
	STA SPRITE.RECORD+$9		;save
...
.DO.CONVERSION
	...
	CMP MAP_OBJECTS.MOB+$0,X	;sprite GMAP X-Axis
	BCC .MOB.MO_X_LESS
	
	SEC
	SBC MAP_OBJECTS.MOB+$0,X	;sprite GMAP X-Axis	
...
.MOB.MO_X_LESS ;(Player X less than sprite X )
	LDA MAP_OBJECTS.MOB+$0,X	;sprite GMAP X-Axis
...
.MOB.YTEST	
	...
	CMP MAP_OBJECTS.MOB+$1,X		;sprite GMAP Y-Axis
	BCC .MOB.MO_Y_LESS
	
	SEC
	SBC MAP_OBJECTS.MOB+$1,X		;sprite GMAP Y-Axis
