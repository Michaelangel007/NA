...
GAME.START.DRIVER ;takes control from NOXARCH.MAIN
@START	
...
@MIDDLE ;MAP POSITION, TALK FILE POINTER, RANDOM.8.INIT
	...
	; JSR COMBAT_SE.STATS.APPLY.PERCENTAGE
		; ;RETURN VALUE: RESULT(2)
				...
				; ; LDY COMBAT_SE.STATS.PERCENT.PARM
				; JSR PREP.BRK
				; BRK
...
;------------------generate seed value--------
	; NOTES: (seeds $4E, $4F with random value)
	;disable for express iteration testing
	;enable when testing things involving random numbers, like combat
	
	;***LONG TERM PLAN: One otion is to do this init via a "floppy players: insert player disk, All players: press a key to continue" prompt
	;		which appears after the player launches the game via the game settings menu. Or if animation ISN'T
	; 	used when the game settings menu is up, the JSR $FD1B could generate the seed and capture keyboard input 
	; 	for the game settings menu.  
	;		see email with Peter Ferrie (in mark.lemmert@gmail.com email box) with subject "ProRWTS2 zero-page question" for an elaboration on this plan. 
	;	    Peter also suggested a way to do it in the animation loop. 
	
	;enable ROM, disable BSR	
		LDA $C082 
...
; .LOOP.RND
			...
			; ;set modulus
			; ;LDA #$FF !255
			; ;LDA #$AB ;!171       ;$B000 = $12  ($19 expected)
			; ;LDA #$57 ;!171       ;$B000 = $16  ($19 expected)
			; ;LDA #$0D ;!171       ;$B000 = $18  ($19 expected)
			; ;LDA #$F3 ;!171       ;$B000 = $0B  ($19 expected)
			; ;LDA #$FF ;!171       ;$B000 = $15  ($19 expected)

			; ;powers of 2
			; LDA #$02				;$B000 = $18  ($19 expected)
			; ;LDA #$04				;$B000 = $19  ($19 expected)
			; ;LDA #$80				;$B000 = $16  ($19 expected)
			; ;LDA #$40				;$B000 = $19  ($19 expected)
		; JSR RANDOM.8
		; STA $A000,X
		; CMP #$E6			;set range
		; BCS .GRE.C0
...
;(GMAP.X/Y can be set to any location on the map that isn't beyond the zone transition column/row in edge zones)
;(i.e. it is not required that GMAP.X/Y be set to a zone's center tile)

	;MAIN ISLAND: GMAP.X=$18, GMAP.Y=$37, ZONE $09: GMAP.X=$18, GMAP.Y=$17
		;LDA #$6F
		LDA #$57	;new island: kfest presentation (by lake)
...
;		STA GMAP.X.LAST. see GAME.LAUNCH
		...
		LDA #$55	;new island: kfest presentation (by lake)
...
@MIDDLE ;PLAYER, TIME & GAME SETTINGS
...
;SETUP PLAYER PARTY
	; LDA #03 ;set party of 3
	; STA PARTY.TOTAL.PC
	;***WARNING: when the game code changes PARTY.TOTAL.PC (i.e. a character is added/removed, then CALCUALTE.COMBAT.STATS needs to be called. presumably this would be done from a routine that does the setup/remove for the new/leaving characters stats) 
	;***WARNING2: for testing the combat module overrides this setting by iterating the PC map object array and counting the # of PCs active.
	
	
;SETUP PLAYER MAP ICON
	;LDA #$B8							;WARRIOR: SPEAR TWIRLER
	LDA #$FF							;GUARD
;	LDA #$99							;HALF SUNK	
;	LDA #$9A							;FULL SUNK	
...
;SET HOTKEYS: SPELLS
	;see PLAYER.HOTKEYS.SPELLS in offloaded_variables.ASM
	;until save game file is setup, I've init the hotkeys using .BS,<$value> directives directly in the variables file. 
...
;SET WORLD
	...
	;save/load from save game file (this is needed to make sure the red/blue floor tile switchover happens corretly if the game was saved when the sun was setting or rising)
	;TIME.SUN.COUNTER
	;TIME.SUN.SUB_COUNTER
...
;SET CURRENT TIME (MILIARY) & 12-HOUR DISPLAY CLOCK
	...
	STA TIME.DISPLAY.AM_PM		;set start time: AM
	
	;set HOUR (24-hour format)
	LDA #$06					;!BCD!set start hour: 6AM
	...
	;set MINUTE
	LDA #$26					;!BCD! set start min : 6:26AM
	;LDA #$00					;!BCD! set start min : 6:00AM
	STA TIME.CURRENT.MINUTE
	STA TIME.DISPLAY.MINUTE
	
	;********************REMOVE THIS THING BELOW****
...
; ;so that the open record value gets set in the player inventory table.
; ;The labels such as INV.PLAYER.INVENTORY.TEST_DATA.END probably also
; ;would be different too. (update 7/1/2017: I don't recall why I was thinking the labels would
; ;be different once this code is moved to the character creation routine)
...
;init miscellanous
	...
	STA MOB.COLLISION_OVERRIDE			;$00 = OFF, $01 = ON
		;STA DRAW.TILE.SHAPE_TABLE.OVERRIDE
...
; ;DEBUG HOOK: ANIMATION PAUSE
; ;(toggle ON/OFF by toggling the hook1)
					; lda troubleshooting.hook
					; cmp #$01
					; bne .temp
;;
...
; ;DEBUG PRINT: 0
			; JSR DEBUG.PRINT ;0
				...
				; ;clear hook. related to .MONITOR.VARIABLE in animation manager. The hook is set at the bottom of .LOAD.NEW.LOCATION in movement mgr. 
				; LDA #$00
				; STA TROUBLESHOOTING.HOOK
...
.CHECK.PLAYTEST_KEYS
	...
	BEQ	.KEY4_STEP	
	CMP #$A4			;SHIFT+4 (LAUNCH TEST INVENTORY MODE)
	BEQ .KEY_SHIFT4_STEP	
...
PLAYTEST.KEY_SHIFT4 ;====LAUNCH INVENTORY TEST MODE===    ;**OPT** Memory. There is no reason to keep this hook active once the inventory module is done
@START
;LOAD INVENTORY FILE INTO THE MAIN MEMORY SWAP SPACE
	JSR SWAP.MAIN_MEMORY.OUT ;swap out anything currently occupying the swap space
	

		LDA #$04 ;(PARM: ACC = inventory sub-module # to launch)
	JSR LAUNCH.INVENTORY.MODULE
	JSR INV_4.DISPLAY_INVENTORY
...
@START
				...
				.IN 	c:\my_code\includes_libs\misc.main_memory_only.asm	
				.IN 	c:\my_code\includes_libs\data.txt.global.asm

				.IN 	c:\my_code\includes_libs\sound_manager.asm
...
;DATA FILES: SHAPES			
				...
				.IN 	c:\my_code\includes_libs\data.shapes.castle_courtyard.asm
				.IN 	c:\my_code\includes_libs\data.shapes.undermap.asm
		;		.IN 	c:\my_code\includes_libs\data.shapes.undermap_town.asm
...
;DATA FILES: LOCATION 2
				;.IN 	c:\my_code\includes_libs\data.tlk.l001.asm
				;floor1.1 (map3)
				.IN 	c:\my_code\includes_libs\data.map.l2.f1.1.m3.asm
				.IN 	c:\my_code\includes_libs\data.spr.l2.f1.1.m3.asm
				;floor2.1 (map4)
				.IN 	c:\my_code\includes_libs\data.map.l2.f2.1.m4.asm
				.IN 	c:\my_code\includes_libs\data.spr.l2.f2.1.m4.asm
				;floor3.1 (map5)
				.IN 	c:\my_code\includes_libs\data.map.l2.f3.1.m5.asm
				.IN 	c:\my_code\includes_libs\data.spr.l2.f3.1.m5.asm
;DATA FILES: UNDERMAP
...
;MAIN GAME FILES				
				...
				.IN 	c:\my_code\includes_libs\swap.routines.cast_spell.setup.asm
				.IN 	c:\my_code\includes_libs\swap.routines.spell_file.asm
				.IN 	c:\my_code\includes_libs\swap.routines.combat.asm
				.IN 	c:\my_code\includes_libs\swap.routines.combat.setup.asm
				.IN 	c:\my_code\includes_libs\swap.routines.combat.exit.asm
				;
				.IN 	c:\my_code\includes_libs\swap.routines.inventory.asm
				.IN 	c:\my_code\includes_libs\swap.routines.npc.talk.asm
				.IN 	c:\my_code\includes_libs\swap.routines.non_building.asm
				.IN 	c:\my_code\includes_libs\swap.routines.building.asm
				;
				;
				...
				.IN 	c:\my_code\includes_libs\bs_aux_routines.bank2.ASM

				
;DATA FILES: GAME & PLAYER
				.IN 	c:\my_code\includes_libs\data.ply.character_sheet.ASM
