...
;loop counters
	LDY #$20				;set initial count X to repeat
decrementmulti:
;;;;;;;;;;;;;; Part 1
...
;Sound 3
  ...
  LDA WAIT.LENGTH 
  SEC
  SBC #$1			;subtract #$20 from WAIT.LENGTH
  STA WAIT.LENGTH 
  ;LSR ;/2 (shift the bits in the binary byte to the left by one bit position, which effectively divides by decimal 2)	
  ; STA WAIT.LENGTH

  
  ;specify how many loop cycles (at the end) have no wait
  CPY #$01 ;# of cycles with no wait
  BCS .DECREMENT.LOOP_COUNTER ;branch if current loop cycle >= Y-REG
  LDA #$00
  STA WAIT.LENGTH 
  
.DECREMENT.LOOP_COUNTER
  DEY
  CPY #$0					;if Y = this, then stop.  next line decements 1
  BNE decrementmulti
  
  
;WAIT FOR KEY PRESS
.KEYIN
	LDA $C000 ;check memory address that holds last key press
    BPL .KEYIN ;if bit7 is set then a key was pressed. So if high bit is set then branch back to .keyin
    STA $C010               ;save keypress to the acknowledgement address, which sets bit 7 on $C000 to zero, effectively clearing the keypress. 

	
;BREAK TO SYSTEM MONITOR
  LDA #$AA
  BRK
  
  
;this table.... 
;WAIT.LENGTH.TABLE FF.80.40.
...
;Sound 3
  ...
  BNE decrement6
	 ...
	 JSR WAIT.LOOP
	  ...
	  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	  ;;;;; Start individual mob hits ;;;;
	  
	  
	LDY #$8				;set initial count X to repeat
decrementmultia:
;;;;;;;;;;;;;; Part 1
	LDX #$8				;set initial count X to repeat
decrementa:
...
;Sound 3
  ...
  BNE decrement2a
   ...
   DEY
  CPY #$3					;if Y = this, then stop.  next line decements 1
  BNE decrementmultia	
